#!/usr/bin/env python3
"""Analyze new holdings against strategy picks."""

# Your new holdings (converted from names)
new_holdings = ['HOOD', 'AVGO', 'CRZBY', 'WBD', 'HOT.DE', 'APP', 'ADBE', 'RWE.DE', 'RHM.DE', 'URTH', 
...
print('=' * 80)
print('📊 NEW HOLDINGS ANALYSIS')
print('=' * 80)

# Categorize holdings
in_strategies = [t for t in new_holdings if t in all_strategy_picks]
not_in_strategies = [t for t in new_holdings if t not in all_strategy_picks]

# Check which strategy each is in
in_risk_adj = [t for t in new_holdings if t in risk_adj_mom_picks]
in_dynamic_bh = [t for t in new_holdings if t in dynamic_bh_1y_picks]

print(f"\n📈 New Holdings: {len(new_holdings)} total")
...
missing = [t for t in all_strategy_picks if t not in new_holdings]
...
print(f"\n📊 SUMMARY:")
print(f"   Portfolio size: {len(new_holdings)} → {len(keep_actions) + len(buy_actions)}")
print(f"   Trades needed: {len(sell_actions)} sells + {len(buy_actions)} buys = {len(sell_actions) + len(buy_actions)} total")

# Notable stocks not in strategies
print(f"\n📌 NOTABLE STOCKS NOT IN STRATEGIES:")
notable = ['AVGO', 'META', 'ADBE', 'LRCX', 'WDC', 'HOOD', 'APP', 'WBD']
for ticker in notable:
    if ticker in not_in_strategies:
        print(f"   {ticker} - Quality stock but not in current top 10")
