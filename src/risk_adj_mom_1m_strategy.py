...
def select_risk_adj_mom_1m_stocks(
    ...
    filtered_tickers = filter_tickers_by_performance(
        all_tickers, ticker_data_grouped, current_date, "Risk-Adj Mom 1M"
    )

    PERF_WINDOW = 30  # 1 month

    candidates = []
    print(f"   📊 Risk-Adj Mom 1M: Analyzing {len(filtered_tickers)} tickers (filtered from {len(all_tickers)})")
    ...
    for ticker in filtered_tickers:
        try:
            ...
            # Momentum confirmation (adjusted for 1M window)
    ...
    if not candidates:
        print(f"   ⚠️ Risk-Adj Mom 1M: No candidates found")
        return []

    candidates.sort(key=lambda x: x[1], reverse=True)
    selected = [t for t, s, r, v in candidates[:top_n]]

    print(f"   ✅ Risk-Adj Mom 1M: Found {len(candidates)} candidates, selected {len(selected)}")
