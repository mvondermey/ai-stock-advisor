...
def _run_portfolio_backtest_walk_forward(
    ...
    for current_date in business_days:
        ...
        if True:  # Print daily summary every day
            ...
            print(f"{'Rank':<5} {'Strategy':<20} {'Value':<13} {'Return':<9} {'StdDev':<9} {'Ann.Ret':<10} {'Cash':<13} {'Positions':<10}")
            print("-" * 100)
            ...
            for i, (name, value, strat_cash, num_pos, invested) in enumerate(strategy_details, 1):
                ...
                print(f"{i:<5} {name:<20} ${value:<12,.0f} {return_pct:+6.1f}%  {std_dev:5.1f}%   {annualized_return:+7.1f}%  ${strat_cash:<12,.0f} {num_pos:<10} ({allocation_pct:.0f}%)")
