...
def _run_parallel_strategy_selection_task(
    ...
    if strategy_name == "elite_hybrid":
    ...
    elif strategy_name == "sma50_1y_delta_ratio":
        from shared_strategies import select_sma50_1y_delta_ratio_stocks

        selected = select_sma50_1y_delta_ratio_stocks(
            initial_top_tickers,
            ticker_data_grouped,
            current_date=current_date,
            top_n=top_n,
    ...
    elif strategy_name == "deep_recovery":
        from deep_recovery_strategy import select_deep_recovery_stocks

        selected = select_deep_recovery_stocks(
            initial_top_tickers,
            ticker_data_grouped,
            current_date=current_date,
            top_n=top_n,
    ...
    elif strategy_name in {"price_curvature", "static_price_curvature"}:
    ...
    elif strategy_name in {"price_curvature_1y_slope", "static_price_curvature_1y_slope"}:
        from price_curvature_strategy import select_price_curvature_1y_slope_stocks

        selected = select_price_curvature_1y_slope_stocks(
            initial_top_tickers,
            ticker_data_grouped,
            current_date=current_date,
            top_n=top_n,
        ...
        selected = select_foresight_mimic_stocks(
            initial_top_tickers,
            ticker_data_grouped,
            current_date=current_date,
            top_n=top_n,
        ...
        selected = select_bh_1y_1m_rank_stocks(
            initial_top_tickers,
            ticker_data_grouped,
            current_date=current_date,
            top_n=top_n,
        ...
        selected = select_bh_1y_6m_rank_stocks(
            initial_top_tickers,
            ticker_data_grouped,
            current_date=current_date,
            top_n=top_n,
        ...
        selected = select_bh_1y_sma200_stocks(
            initial_top_tickers,
            ticker_data_grouped,
            current_date=current_date,
            top_n=top_n,
        ...
        selected = select_bh_1y_fcf_rank_stocks(
            initial_top_tickers,
            ticker_data_grouped,
            current_date=current_date,
            top_n=top_n,
...
from config import (
    ...
    STATIC_PRICE_CURVATURE_REBALANCE_DAYS, STATIC_PRICE_CURVATURE_1Y_SLOPE_REBALANCE_DAYS,
    ...
    STATIC_BH_6M_PERFORMANCE_THRESHOLD, STATIC_BH_9M_PERFORMANCE_THRESHOLD,
...
STRATEGY_REGISTRY = {
    ...
    'sma50_1y_delta_ratio': _strategy_registry_entry('SMA50 / 6M Delta', 'ENABLE_SMA50_1Y_DELTA_RATIO', 'sma50_1y_delta_ratio_portfolio_value', 'sma50_1y_delta_ratio_portfolio_history', 'sma50_1y_delta_ratio_cash', 'sma50_1y_delta_ratio_positions'),
    ...
    'static_price_curvature': _strategy_registry_entry('Static Price Curvature', 'ENABLE_STATIC_PRICE_CURVATURE', 'static_price_curvature_portfolio_value', 'static_price_curvature_portfolio_history', 'static_price_curvature_cash', 'current_static_price_curvature_stocks'),
    ...
    'static_price_curvature_1y_slope': _strategy_registry_entry('Static Price Curvature 1Y Slope', 'ENABLE_STATIC_PRICE_CURVATURE_1Y_SLOPE', 'static_price_curvature_1y_slope_portfolio_value', 'static_price_curvature_1y_slope_portfolio_history', 'static_price_curvature_1y_slope_cash', 'current_static_price_curvature_1y_slope_stocks'),
    'price_curvature_1y_slope': _strategy_registry_entry('Price Curvature 1Y Slope', 'ENABLE_PRICE_CURVATURE_1Y_SLOPE', 'price_curvature_1y_slope_portfolio_value', 'price_curvature_1y_slope_portfolio_history', 'price_curvature_1y_slope_cash', 'current_price_curvature_1y_slope_stocks'),
    ...
    'foresight_mimic': _strategy_registry_entry('Foresight Mimic Accel', 'ENABLE_FORESIGHT_MIMIC', 'foresight_mimic_portfolio_value', 'foresight_mimic_portfolio_history', 'foresight_mimic_cash', 'current_foresight_mimic_stocks'),
    ...
    'deep_recovery': _strategy_registry_entry('Deep Recovery', 'ENABLE_DEEP_RECOVERY', 'deep_recovery_portfolio_value', 'deep_recovery_portfolio_history', 'deep_recovery_cash', 'current_deep_recovery_stocks'),
    ...
    'static_bh_6m_perf': _strategy_registry_entry('BH 6M Perf Trig', 'ENABLE_STATIC_BH_6M_PERFORMANCE', 'static_bh_6m_perf_portfolio_value', 'static_bh_6m_perf_portfolio_history', 'static_bh_6m_perf_cash', 'static_bh_6m_perf_positions'),
    'static_bh_9m_perf': _strategy_registry_entry('BH 9M Perf Trig', 'ENABLE_STATIC_BH_9M_PERFORMANCE', 'static_bh_9m_perf_portfolio_value', 'static_bh_9m_perf_portfolio_history', 'static_bh_9m_perf_cash', 'static_bh_9m_perf_positions'),
...
ratio_1m_3m_transaction_costs = 0
sma50_1y_delta_ratio_transaction_costs = 0
turnaround_transaction_costs = 0
deep_recovery_transaction_costs = 0
...
voting_ensemble_ai_rebalance_transaction_costs = 0
momentum_volatility_hybrid_transaction_costs = 0
momentum_volatility_hybrid_6m_transaction_costs = 0
static_price_curvature_transaction_costs = 0
price_curvature_transaction_costs = 0
static_price_curvature_1y_slope_transaction_costs = 0
price_curvature_1y_slope_transaction_costs = 0
foresight_mimic_transaction_costs = 0
bh_1y_1m_rank_transaction_costs = 0
bh_1y_6m_rank_transaction_costs = 0
bh_1y_6m_blend_transaction_costs = 0
early_leader_accel_transaction_costs = 0
bh_1y_sma200_transaction_costs = 0
bh_1y_fcf_rank_transaction_costs = 0
...
def _build_daily_close_price_cache(
    ticker_data_grouped: Dict[str, pd.DataFrame],
    current_date: datetime,
) -> Dict[str, float]:
    """Precompute the latest valid close for each ticker once per backtest day."""
    close_price_cache: Dict[str, float] = {}
    for ticker, ticker_df in ticker_data_grouped.items():
        close_price = _last_valid_close_up_to(ticker_df, current_date)
        if close_price is not None:
            close_price_cache[ticker] = close_price
    return close_price_cache
...
def _return_between_dates(
    ...
    """Return over an exact date window using first/last valid closes inside it."""
    try:
        window = ticker_df.loc[start_date:end_date, "Close"].dropna()
        if len(window) < 2:
            return None
        start_price = float(window.iloc[0])
        end_price = float(window.iloc[-1])
...
def _select_top_performers_over_window(
    ...
    """Select hindsight top performers over the exact backtest window."""
    scored_tickers = []
    for ticker in all_tickers:
        ticker_df = ticker_data_grouped.get(ticker)
        if ticker_df is None:
            continue
        total_return = _return_between_dates(ticker_df, start_date, end_date)
        if total_return is not None:
            scored_tickers.append((ticker, total_return))
    scored_tickers.sort(key=lambda item: item[1], reverse=True)
    return [ticker for ticker, _ in scored_tickers[:top_n]]
...
def _should_rebalance_by_profit_since_last_rebalance(
    ...
    portfolio_value_now = _mark_to_market_value(
        positions,
        cash,
        ticker_data_grouped,
        current_date,
    ...
    *,
    ...
    if positions_to_actually_buy:
        held_positions_value = sum(
            updated_positions[ticker]['shares'] * close_price_cache[ticker]
...
def _run_portfolio_backtest_walk_forward(
    ...
    from shared_strategies import (
        ...
        select_quality_momentum_stocks as _select_quality_momentum_stocks_impl,
        select_risk_adj_mom_stocks as _select_risk_adj_mom_stocks_impl,
        select_top_performers as _select_top_performers_impl,
        select_top_performers_vol_filtered as _select_top_performers_vol_filtered_impl,
    )
    clear_inverse_etf_hedge_log()
    ...
    parallel_strategy_display_names = {
        ...
        "sma50_1y_delta_ratio": "SMA50 / 6M Delta",
    ...
    benchmark_top_hold_last_rebalance_value = initial_capital_needed

    # Hindsight benchmark: choose the best performers over the backtest window,
    # buy them at the start, and hold without rebalancing.
    benchmark_top_start_name = f"Perfect Foresight Top {PORTFOLIO_SIZE}"
    benchmark_top_start_portfolio_value = initial_capital_needed
    benchmark_top_start_portfolio_history = [benchmark_top_start_portfolio_value]
    ...
    # 2b. Performance-triggered (6M ranking)
    static_bh_6m_perf_portfolio_value = initial_capital_needed
    static_bh_6m_perf_portfolio_history = [static_bh_6m_perf_portfolio_value]
    static_bh_6m_perf_positions = {}
    static_bh_6m_perf_cash = initial_capital_needed
    current_static_bh_6m_perf_stocks = []
    static_bh_6m_perf_initialized = False
    static_bh_6m_perf_transaction_costs = 0.0

    # 2c. Performance-triggered (9M ranking)
    static_bh_9m_perf_portfolio_value = initial_capital_needed
    static_bh_9m_perf_portfolio_history = [static_bh_9m_perf_portfolio_value]
    static_bh_9m_perf_positions = {}
    static_bh_9m_perf_cash = initial_capital_needed
    current_static_bh_9m_perf_stocks = []
    static_bh_9m_perf_initialized = False
    static_bh_9m_perf_transaction_costs = 0.0
    ...
    # SMA50 / 1Y DELTA RATIO: Initialize portfolio tracking
    sma50_1y_delta_ratio_portfolio_value = initial_capital_needed
    sma50_1y_delta_ratio_portfolio_history = [sma50_1y_delta_ratio_portfolio_value]
    sma50_1y_delta_ratio_positions = {}
    sma50_1y_delta_ratio_cash = initial_capital_needed
    current_sma50_1y_delta_ratio_stocks = []
    sma50_1y_delta_ratio_last_rebalance_value = initial_capital_needed
    ...
    # STATIC PRICE CURVATURE: Initialize portfolio tracking
    static_price_curvature_portfolio_value = initial_capital_needed
    static_price_curvature_portfolio_history = [static_price_curvature_portfolio_value]
    static_price_curvature_positions = {}  # ticker -> {'shares': float, 'entry_price': float, 'value': float}
    static_price_curvature_cash = initial_capital_needed
    current_static_price_curvature_stocks = []
    static_price_curvature_days_since_rebalance = 0
    static_price_curvature_initialized = False
    ...
    # STATIC PRICE CURVATURE 1Y SLOPE: Initialize portfolio tracking
    static_price_curvature_1y_slope_portfolio_value = initial_capital_needed
    static_price_curvature_1y_slope_portfolio_history = [static_price_curvature_1y_slope_portfolio_value]
    static_price_curvature_1y_slope_positions = {}
    static_price_curvature_1y_slope_cash = initial_capital_needed
    current_static_price_curvature_1y_slope_stocks = []
    static_price_curvature_1y_slope_days_since_rebalance = 0
    static_price_curvature_1y_slope_initialized = False

    # PRICE CURVATURE 1Y SLOPE: Initialize portfolio tracking
    price_curvature_1y_slope_portfolio_value = initial_capital_needed
    price_curvature_1y_slope_portfolio_history = [price_curvature_1y_slope_portfolio_value]
    price_curvature_1y_slope_positions = {}
    price_curvature_1y_slope_cash = initial_capital_needed
    current_price_curvature_1y_slope_stocks = []
    price_curvature_1y_slope_last_rebalance_value = initial_capital_needed
    ...
    # FORESIGHT MIMIC: Initialize portfolio tracking
    foresight_mimic_portfolio_value = initial_capital_needed
    foresight_mimic_portfolio_history = [foresight_mimic_portfolio_value]
    foresight_mimic_positions = {}
    foresight_mimic_cash = initial_capital_needed
    current_foresight_mimic_stocks = []
    foresight_mimic_last_rebalance_value = initial_capital_needed

    # BH 1Y / 1M RANK: Initialize portfolio tracking
    bh_1y_1m_rank_portfolio_value = initial_capital_needed
    bh_1y_1m_rank_portfolio_history = [bh_1y_1m_rank_portfolio_value]
    bh_1y_1m_rank_positions = {}
    bh_1y_1m_rank_cash = initial_capital_needed
    current_bh_1y_1m_rank_stocks = []
    bh_1y_1m_rank_last_rebalance_value = initial_capital_needed

    # BH 1Y SMA200: Initialize portfolio tracking
    bh_1y_sma200_portfolio_value = initial_capital_needed
    bh_1y_sma200_portfolio_history = [bh_1y_sma200_portfolio_value]
    bh_1y_sma200_positions = {}
    bh_1y_sma200_cash = initial_capital_needed
    current_bh_1y_sma200_stocks = []
    bh_1y_sma200_last_rebalance_value = initial_capital_needed

    # BH 1Y / FCF RANK: Initialize portfolio tracking
    bh_1y_fcf_rank_portfolio_value = initial_capital_needed
    bh_1y_fcf_rank_portfolio_history = [bh_1y_fcf_rank_portfolio_value]
    bh_1y_fcf_rank_positions = {}
    bh_1y_fcf_rank_cash = initial_capital_needed
    current_bh_1y_fcf_rank_stocks = []
    bh_1y_fcf_rank_last_rebalance_value = initial_capital_needed

    # BH 1Y / 6M RANK: Initialize portfolio tracking
    bh_1y_6m_rank_portfolio_value = initial_capital_needed
    bh_1y_6m_rank_portfolio_history = [bh_1y_6m_rank_portfolio_value]
    bh_1y_6m_rank_positions = {}
    bh_1y_6m_rank_cash = initial_capital_needed
    current_bh_1y_6m_rank_stocks = []
    bh_1y_6m_rank_last_rebalance_value = initial_capital_needed

    # BH 1Y / 6M BLEND: Initialize portfolio tracking
    bh_1y_6m_blend_portfolio_value = initial_capital_needed
    bh_1y_6m_blend_portfolio_history = [bh_1y_6m_blend_portfolio_value]
    bh_1y_6m_blend_positions = {}
    ...
    early_leader_accel_portfolio_history = [early_leader_accel_portfolio_value]
    early_leader_accel_positions = {}
    early_leader_accel_cash = initial_capital_needed
    current_early_leader_accel_stocks = []
    early_leader_accel_last_rebalance_value = initial_capital_needed
    ...
    # DEEP RECOVERY: Initialize portfolio tracking
    deep_recovery_portfolio_value = initial_capital_needed
    deep_recovery_portfolio_history = [deep_recovery_portfolio_value]
    deep_recovery_positions = {}  # ticker -> {'shares': float, 'entry_price': float, 'value': float}
    deep_recovery_cash = initial_capital_needed
    current_deep_recovery_stocks = []
    deep_recovery_last_rebalance_value = initial_capital_needed
    deep_recovery_transaction_costs = 0.0
    ...
    # VOTING ENSEMBLE + AI REBALANCE: same selection, separate AI-driven replace decisions
    voting_ensemble_ai_rebalance_portfolio_value = initial_capital_needed
    voting_ensemble_ai_rebalance_portfolio_history = [voting_ensemble_ai_rebalance_portfolio_value] if config.ENABLE_VOTING_ENSEMBLE_AI_REBALANCE else []
    voting_ensemble_ai_rebalance_positions = {}
    voting_ensemble_ai_rebalance_cash = initial_capital_needed
    current_voting_ensemble_ai_rebalance_stocks = []
    voting_ensemble_ai_rebalance_transaction_costs = 0.0
    voting_ensemble_ai_rebalance_model = None
    voting_ensemble_ai_rebalance_last_train_day = 0
    voting_ensemble_ai_rebalance_cache_context = None
    ...
    ai_elite_monthly_shared_positions = {}
    ai_elite_monthly_shared_cash = initial_capital_needed
    current_ai_elite_monthly_shared_stocks = []
    ai_elite_monthly_shared_transaction_costs = 0.0
    ai_elite_monthly_shared_initialized = False
    ai_elite_monthly_shared_last_month = None
    ...
    # AI ELITE ENSEMBLE: Initialize portfolio tracking (weighted avg of top 3 positive-R² models)
    ai_elite_ensemble_portfolio_value = initial_capital_needed
    ai_elite_ensemble_portfolio_history = [ai_elite_ensemble_portfolio_value] if config.ENABLE_AI_ELITE_ENSEMBLE else []
    ai_elite_ensemble_positions = {}
    ai_elite_ensemble_cash = initial_capital_needed
    current_ai_elite_ensemble_stocks = []
    ai_elite_ensemble_transaction_costs = 0.0
    ai_elite_ensemble_initialized = False

    # AI ELITE RANK ENSEMBLE: Initialize portfolio tracking (weighted rank avg of top 3 positive-R² models)
    ai_elite_rank_ensemble_portfolio_value = initial_capital_needed
    ai_elite_rank_ensemble_portfolio_history = [ai_elite_rank_ensemble_portfolio_value] if config.ENABLE_AI_ELITE_RANK_ENSEMBLE else []
    ai_elite_rank_ensemble_positions = {}
    ai_elite_rank_ensemble_cash = initial_capital_needed
    current_ai_elite_rank_ensemble_stocks = []
    ...
    global dynamic_bh_3m_transaction_costs, dynamic_bh_6m_transaction_costs, dynamic_bh_1m_transaction_costs, risk_adj_mom_transaction_costs, mean_reversion_transaction_costs, quality_momentum_transaction_costs, momentum_ai_hybrid_transaction_costs, volatility_adj_mom_transaction_costs, dynamic_bh_1y_vol_filter_transaction_costs, dynamic_bh_1y_trailing_stop_transaction_costs, ratio_3m_1y_transaction_costs, ratio_1y_3m_transaction_costs, ratio_1m_3m_transaction_costs, sma50_1y_delta_ratio_transaction_costs, turnaround_transaction_costs, adaptive_ensemble_transaction_costs, volatility_ensemble_transaction_costs, sentiment_ensemble_transaction_costs
    ...
    sma50_1y_delta_ratio_transaction_costs = 0.0
    ...
    momentum_volatility_hybrid_1y_transaction_costs = 0.0
    momentum_volatility_hybrid_1y3m_transaction_costs = 0.0
    static_price_curvature_transaction_costs = 0.0
    price_curvature_transaction_costs = 0.0
    static_price_curvature_1y_slope_transaction_costs = 0.0
    price_curvature_1y_slope_transaction_costs = 0.0
    confirmed_leaders_transaction_costs = 0.0
    foresight_mimic_transaction_costs = 0.0
    bh_1y_1m_rank_transaction_costs = 0.0
    bh_1y_6m_rank_transaction_costs = 0.0
    bh_1y_sma200_transaction_costs = 0.0
    bh_1y_fcf_rank_transaction_costs = 0.0
    ...
    if config.ENABLE_PARALLEL_STRATEGIES:
        try:
            ...
            parallel_strategy_executor = None
            print(f"   ⚠️ Parallel strategy queue disabled: {e}", flush=True)
    ...
    active_close_price_cache = _build_daily_close_price_cache(ticker_data_grouped, benchmark_start_date)
    close_cache_ticker_by_df_id = {id(ticker_df): ticker for ticker, ticker_df in ticker_data_grouped.items()}

    _mark_to_market_value_impl = globals()['_mark_to_market_value']
    _smart_rebalance_portfolio_impl = globals()['_smart_rebalance_portfolio']
    _should_rebalance_by_profit_since_last_rebalance_impl = globals()['_should_rebalance_by_profit_since_last_rebalance']

    def _last_valid_close_up_to(ticker_df: pd.DataFrame, current_date: datetime) -> Optional[float]:
        ticker = close_cache_ticker_by_df_id.get(id(ticker_df))
        if ticker is None:
            return None
        return active_close_price_cache.get(ticker)
    ...
    def _mark_to_market_value(
        ...
        return _mark_to_market_value_impl(
            positions,
            cash,
            ticker_data_grouped,
            current_date,
    ...
    def _smart_rebalance_portfolio(
        ...
        result = _smart_rebalance_portfolio_impl(
            ...
            use_atr_trailing_stop=use_atr_trailing_stop,
    ...
    def _ai_rebalance_portfolio(
        ...
        ai_ranked_candidates, decision_logs = choose_ai_rebalance_ranked_candidates(
            current_holdings=current_stocks,
        ...
        positions, cash, final_stocks, costs, rebalanced = _smart_rebalance_portfolio(
            strategy_name=strategy_name,
            current_stocks=current_stocks,
            new_stocks=ai_ranked_candidates,
            positions=positions,
            cash=cash,
            ticker_data_grouped=ticker_data_grouped,
            current_date=current_date,
    ...
    def _should_rebalance_by_profit_since_last_rebalance(
        ...
        return _should_rebalance_by_profit_since_last_rebalance_impl(
            current_stocks=current_stocks,
            new_stocks=new_stocks,
            ticker_data_grouped=ticker_data_grouped,
            current_date=current_date,
            ...
            cash=cash,
            transaction_cost=transaction_cost,
            last_rebalance_value=last_rebalance_value,
            close_price_cache=active_close_price_cache,
        )
    ...
    def select_quality_momentum_stocks(
        all_tickers,
        ticker_data_grouped,
        current_date=None,
        top_n=20,
    ...
    def _update_portfolio_value_from_cache(
        positions: Dict[str, Dict],
        cash: float,
        ...
        if debug_message:
            print(debug_message)
        for ticker in list((positions or {}).keys()):
            pos = positions[ticker]
            try:
                ticker_df = ticker_data_grouped.get(ticker)
                if ticker_df is not None:
                    ...
                    if current_price is not None and not pd.isna(current_price) and current_price > 0:
                        ...
                        continue
                invested += pos.get('value', 0.0)
            except Exception as e:
                if error_prefix:
                    print(f"   ⚠️ {error_prefix} for {ticker}: {e}")
                invested += pos.get('value', 0.0)
        return invested + cash

    benchmark_top_start_stocks = _select_top_performers_over_window(
        initial_top_tickers,
        ticker_data_grouped,
    ...
    if benchmark_top_start_stocks:
        ...
        print(
            f"   📌 {benchmark_top_start_name}: Hindsight-selected holdings over "
            f"{benchmark_start_date.strftime('%Y-%m-%d')} to {backtest_end_date.strftime('%Y-%m-%d')} = "
    ...
    for current_date in business_days:
        ...
        dispatched_parallel_strategy_names: set[str] = set()
        ...
        def _dispatch_parallel_strategy_tasks() -> None:
            ...
            for strategy_name, strategy_config in getattr(config, "_PARALLEL_STRATEGY_PILOT_CONFIG", {}).items():
                ...
                dispatched_parallel_strategy_names.add(strategy_name)
        ...
        def _resolve_parallel_strategy_selection(strategy_name: str, fallback_fn=None):
            ...
            def _run_fallback(reason: str, *, warn: bool = True, count_queue_fallback: bool = True):
                if count_queue_fallback:
                ...
                return fallback_fn()
            if future is None:
                if strategy_name in dispatched_parallel_strategy_names:
                    return _run_fallback("missing result")
                return _run_fallback("not queued", warn=False, count_queue_fallback=False)
            try:
        ...
        if config.ENABLE_VOTING_ENSEMBLE_AI_REBALANCE:
            try:
                from ai_rebalance_strategy import (
                    _collect_rebalance_data_worker,
                    init_rebalance_collection_worker,
                    load_voting_ai_rebalance_model,
                    precompute_rebalance_training_context,
                    train_voting_ai_rebalance_model,
                )
                ...
                if should_train_voting_ai_rebalance:
                    ...
                    voting_ensemble_ai_rebalance_cache_context = rebalance_context
                    rebalance_context_tickers = list(rebalance_context.get("all_tickers") or [])

                    collect_args = [
                        (
                            t,
                            rebalance_context_tickers,
                            train_start,
                            current_date,
                            config.AI_VOTING_REBALANCE_FORWARD_DAYS,
                            TRANSACTION_COST,
                            PORTFOLIO_SIZE,
                            PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                        )
                        for t in rebalance_context_tickers
                    ]
        ...
        if config.ENABLE_STATIC_BH_1Y_PERFORMANCE:
            should_rebalance_perf = (not static_bh_1y_perf_initialized) or check_performance_trigger(
                ...
                threshold=config.STATIC_BH_1Y_PERFORMANCE_THRESHOLD
            )
        ...
        # 2b. Performance-triggered rebalancing (6M ranking)
        if config.ENABLE_STATIC_BH_6M_PERFORMANCE:
            should_rebalance_6m_perf = (not static_bh_6m_perf_initialized) or check_performance_trigger(
                static_bh_6m_perf_positions, ticker_data_grouped, current_date,
                threshold=STATIC_BH_6M_PERFORMANCE_THRESHOLD
            )
            if should_rebalance_6m_perf:
                new_stocks = select_top_performers(
                    initial_top_tickers, ticker_data_grouped, current_date, lookback_days=180,
                    top_n=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                    timing_strategy_name="BH 6M Perf Trig",
                )
                if new_stocks:
                    print(f"   📊 BH 6M Perf Trig Day {day_count}: {new_stocks}")
                    if not static_bh_6m_perf_initialized:
                        print(f"   🎯 BH 6M Perf Trig: Initializing")
                    else:
                        print(f"   🔄 BH 6M Perf Trig: Performance trigger rebalance")
                    static_bh_6m_perf_positions, static_bh_6m_perf_cash, current_static_bh_6m_perf_stocks, rc, _ = _smart_rebalance_portfolio(
                        strategy_name="BH 6M Perf Trig", current_stocks=current_static_bh_6m_perf_stocks,
                        new_stocks=new_stocks, positions=static_bh_6m_perf_positions, cash=static_bh_6m_perf_cash,
                        ticker_data_grouped=ticker_data_grouped, current_date=current_date,
                        transaction_cost=TRANSACTION_COST, portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not static_bh_6m_perf_initialized
                    )
                    static_bh_6m_perf_transaction_costs += rc
                    static_bh_6m_perf_initialized = True

        # 2c. Performance-triggered rebalancing (9M ranking)
        if config.ENABLE_STATIC_BH_9M_PERFORMANCE:
            should_rebalance_9m_perf = (not static_bh_9m_perf_initialized) or check_performance_trigger(
                static_bh_9m_perf_positions, ticker_data_grouped, current_date,
                threshold=STATIC_BH_9M_PERFORMANCE_THRESHOLD
            )
            if should_rebalance_9m_perf:
                new_stocks = select_top_performers(
                    initial_top_tickers, ticker_data_grouped, current_date, lookback_days=270,
                    top_n=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                    timing_strategy_name="BH 9M Perf Trig",
                )
                if new_stocks:
                    print(f"   📊 BH 9M Perf Trig Day {day_count}: {new_stocks}")
                    if not static_bh_9m_perf_initialized:
                        print(f"   🎯 BH 9M Perf Trig: Initializing")
                    else:
                        print(f"   🔄 BH 9M Perf Trig: Performance trigger rebalance")
                    static_bh_9m_perf_positions, static_bh_9m_perf_cash, current_static_bh_9m_perf_stocks, rc, _ = _smart_rebalance_portfolio(
                        strategy_name="BH 9M Perf Trig", current_stocks=current_static_bh_9m_perf_stocks,
                        new_stocks=new_stocks, positions=static_bh_9m_perf_positions, cash=static_bh_9m_perf_cash,
                        ticker_data_grouped=ticker_data_grouped, current_date=current_date,
                        transaction_cost=TRANSACTION_COST, portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not static_bh_9m_perf_initialized
                    )
                    static_bh_9m_perf_transaction_costs += rc
                    static_bh_9m_perf_initialized = True
        ...
        if config.ENABLE_STATIC_BH_1Y_VOLUME_FILTER:
            ...
            new_stocks = _time_strategy_phase_call(
                "Static BH 1Y Volume",
                "selection",
                select_volume_filtered_bh_1y_stocks,
                initial_top_tickers,
                ticker_data_grouped,
                current_date,
                PORTFOLIO_SIZE,
        ...
        if config.ENABLE_STATIC_BH_1Y_SECTOR_ROTATION:
            ...
            new_stocks = _time_strategy_phase_call(
                "Static BH 1Y Sector",
                "selection",
                select_sector_rotated_bh_1y_stocks,
                initial_top_tickers,
                ticker_data_grouped,
                current_date,
                PORTFOLIO_SIZE,
        ...
        if config.ENABLE_STATIC_BH_1Y_PERFORMANCE_THRESHOLD:
            ...
            new_stocks, should_rebalance = _time_strategy_phase_call(
                "Static BH 1Y Perf Thresh",
                "selection",
                select_performance_threshold_bh_1y_stocks,
                initial_top_tickers,
                ticker_data_grouped,
                current_date,
                current_static_bh_1y_perf_threshold_stocks,
        ...
        if config.ENABLE_STATIC_BH_1Y_MARKET_REGIME:
            ...
            if not static_bh_1y_market_regime_initialized or static_bh_1y_market_regime_days_since_rebalance >= static_bh_1y_market_regime_next_rebalance_days:
                new_stocks, next_rebalance_days = _time_strategy_phase_call(
                    "Static BH 1Y Market Regime",
                    "selection",
                    select_market_regime_bh_1y_stocks,
                    initial_top_tickers,
                    ticker_data_grouped,
                    current_date,
                    PORTFOLIO_SIZE,
        ...
        if config.ENABLE_STATIC_BH_1Y_MOMENTUM_PERSIST:
            ...
            new_stocks, should_rebalance, static_bh_1y_mom_persist_top_stocks_history = _time_strategy_phase_call(
                "Static BH 1Y Mom Persist",
                "selection",
                select_momentum_persistence_bh_1y_stocks,
                initial_top_tickers,
                ticker_data_grouped,
                current_date,
                static_bh_1y_mom_persist_top_stocks_history,
                current_static_bh_1y_mom_persist_stocks,
                PORTFOLIO_SIZE,
                STATIC_BH_1Y_MOMENTUM_PERSIST_DAYS,
                STATIC_BH_1Y_MOMENTUM_PERSIST_MIN_OVERLAP,
                price_history_cache=price_history_cache,
            )
        ...
        if config.ENABLE_STATIC_BH_1Y_OVERLAP:
            ...
            new_stocks, should_rebalance = _time_strategy_phase_call(
                "Static BH 1Y Overlap",
                "selection",
                select_overlap_based_bh_1y_stocks,
                initial_top_tickers,
                ticker_data_grouped,
                current_date,
                current_static_bh_1y_overlap_stocks,
        ...
        if config.ENABLE_STATIC_BH_1Y_RANK_DRIFT:
            ...
            new_stocks, should_rebalance, static_bh_1y_rank_drift_previous_rankings = _time_strategy_phase_call(
                "Static BH 1Y Rank Drift",
                "selection",
                select_rank_drift_bh_1y_stocks,
                initial_top_tickers,
                ticker_data_grouped,
                current_date,
                static_bh_1y_rank_drift_previous_rankings,
                current_static_bh_1y_rank_drift_stocks,
                PORTFOLIO_SIZE,
                STATIC_BH_1Y_RANK_DRIFT_THRESHOLD,
                price_history_cache=price_history_cache,
            )
            # Force initialization on first run
        ...
        if config.ENABLE_STATIC_BH_1Y_DRAWDOWN:
            ...
            new_stocks, should_rebalance = _time_strategy_phase_call(
                "Static BH 1Y Drawdown",
                "selection",
                select_drawdown_trigger_bh_1y_stocks,
                initial_top_tickers,
                ticker_data_grouped,
                current_date,
                current_static_bh_1y_drawdown_stocks,
                static_bh_1y_drawdown_portfolio_value,
                static_bh_1y_drawdown_peak,
                PORTFOLIO_SIZE,
                STATIC_BH_1Y_DRAWDOWN_THRESHOLD,
                price_history_cache=price_history_cache,
            )
            # Force initialization on first run
        ...
        if config.ENABLE_STATIC_BH_1Y_SMART_MONTHLY:
            ...
            new_stocks, should_rebalance, static_bh_1y_smart_monthly_last_rebalance_month = _time_strategy_phase_call(
                "Static BH 1Y Smart Monthly",
                "selection",
                select_smart_monthly_bh_1y_stocks,
                initial_top_tickers,
                ticker_data_grouped,
                current_date,
                current_static_bh_1y_smart_monthly_stocks,
                static_bh_1y_smart_monthly_portfolio_value,
                static_bh_1y_smart_monthly_peak,
                static_bh_1y_smart_monthly_last_rebalance_month,
                PORTFOLIO_SIZE,
                STATIC_BH_1Y_SMART_MONTHLY_DRAWDOWN,
                price_history_cache=price_history_cache,
            )
        ...
        if config.ENABLE_BH_1Y_TRAILING_MOM:
            ...
            for ticker in bh_1y_trailing_mom_positions:
                if ticker in ticker_data_grouped:
                    current_price = _last_valid_close_up_to(ticker_data_grouped[ticker], current_date)
                    if current_price is not None:
        ...
        if config.ENABLE_RATIO_1M3M_VOL_ADJ_REBAL:
            ...
            if should_init_or_rebalance:
                ...
                new_stocks = _time_strategy_phase_call(
                    "Rebal 1M3M VolAdj",
                    "selection",
                    select_1y_performers_ranked_by_1m3m_ratio,
                    initial_top_tickers,
                    ticker_data_grouped,
                    current_date,
                    top_n=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
        ...
        if config.ENABLE_STATIC_PRICE_CURVATURE:
            static_price_curvature_days_since_rebalance += 1

            should_init_or_rebalance_static_price_curvature = (
                (not static_price_curvature_initialized) or
                (
                    STATIC_PRICE_CURVATURE_REBALANCE_DAYS > 0
                    and static_price_curvature_days_since_rebalance >= STATIC_PRICE_CURVATURE_REBALANCE_DAYS
                )
            )

            if should_init_or_rebalance_static_price_curvature:
                from price_curvature_strategy import select_price_curvature_stocks

                new_static_price_curvature_stocks = select_price_curvature_stocks(
                    initial_top_tickers,
                    ticker_data_grouped,
                    current_date=current_date,
                    top_n=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                ...
                if new_static_price_curvature_stocks:
                    print(f"   📊 Static Price Curvature Day {day_count}: {new_static_price_curvature_stocks}")
                    if not static_price_curvature_initialized:
                        print(f"   🎯 Static Price Curvature: Initializing with {new_static_price_curvature_stocks}")
                    else:
                        print(f"   🔄 Static Price Curvature: Rebalancing to {new_static_price_curvature_stocks}")

                    static_price_curvature_positions, static_price_curvature_cash, current_static_price_curvature_stocks, rebalance_costs, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="Static Price Curvature",
                        current_stocks=current_static_price_curvature_stocks,
                        new_stocks=new_static_price_curvature_stocks,
                        positions=static_price_curvature_positions,
                        cash=static_price_curvature_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not static_price_curvature_initialized,
                    )
                    strategies_rebalanced_today['Static Price Curvature'] = rebalanced_flag
                    static_price_curvature_transaction_costs += rebalance_costs
                    static_price_curvature_initialized = True
                    static_price_curvature_days_since_rebalance = 0
                else:
                    print("   ❌ No Static Price Curvature stocks selected")

        if config.ENABLE_STATIC_PRICE_CURVATURE_1Y_SLOPE:
            static_price_curvature_1y_slope_days_since_rebalance += 1

            should_init_or_rebalance_static_price_curvature_1y_slope = (
                (not static_price_curvature_1y_slope_initialized) or
                (
                    STATIC_PRICE_CURVATURE_1Y_SLOPE_REBALANCE_DAYS > 0
                    and static_price_curvature_1y_slope_days_since_rebalance >= STATIC_PRICE_CURVATURE_1Y_SLOPE_REBALANCE_DAYS
                )
            )

            if should_init_or_rebalance_static_price_curvature_1y_slope:
                from price_curvature_strategy import select_price_curvature_1y_slope_stocks

                new_static_price_curvature_1y_slope_stocks = select_price_curvature_1y_slope_stocks(
                    initial_top_tickers,
                    ticker_data_grouped,
                    current_date=current_date,
                    top_n=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                ...
                if new_static_price_curvature_1y_slope_stocks:
                    print(f"   📊 Static Price Curvature 1Y Slope Day {day_count}: {new_static_price_curvature_1y_slope_stocks}")
                    if not static_price_curvature_1y_slope_initialized:
                        print(f"   🎯 Static Price Curvature 1Y Slope: Initializing with {new_static_price_curvature_1y_slope_stocks}")
                    else:
                        print(f"   🔄 Static Price Curvature 1Y Slope: Rebalancing to {new_static_price_curvature_1y_slope_stocks}")

                    static_price_curvature_1y_slope_positions, static_price_curvature_1y_slope_cash, current_static_price_curvature_1y_slope_stocks, rebalance_costs, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="Static Price Curvature 1Y Slope",
                        current_stocks=current_static_price_curvature_1y_slope_stocks,
                        new_stocks=new_static_price_curvature_1y_slope_stocks,
                        positions=static_price_curvature_1y_slope_positions,
                        cash=static_price_curvature_1y_slope_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not static_price_curvature_1y_slope_initialized,
                    )
                    strategies_rebalanced_today['Static Price Curvature 1Y Slope'] = rebalanced_flag
                    static_price_curvature_1y_slope_transaction_costs += rebalance_costs
                    static_price_curvature_1y_slope_initialized = True
                    static_price_curvature_1y_slope_days_since_rebalance = 0
                else:
                    print("   ❌ No Static Price Curvature 1Y Slope stocks selected")
        ...
        # SMA50 / 6M DELTA RATIO: Rebalance to highest ratio stocks DAILY
        if config.ENABLE_SMA50_1Y_DELTA_RATIO:
            try:
                print(f"   📊 SMA50 / 6M Delta Strategy: Analyzing {len(initial_top_tickers)} tickers on {current_date.strftime('%Y-%m-%d')}")

                def _fallback_sma50_1y_delta_ratio():
                    from shared_strategies import select_sma50_1y_delta_ratio_stocks
                    return select_sma50_1y_delta_ratio_stocks(
                        initial_top_tickers,
                        ticker_data_grouped,
                        current_date,
                        top_n=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                ...
                new_sma50_1y_delta_ratio_stocks = _resolve_parallel_strategy_selection(
                    "sma50_1y_delta_ratio",
                    _fallback_sma50_1y_delta_ratio,
                )

                if new_sma50_1y_delta_ratio_stocks:
                    print(f"   📊 SMA50 / 6M Delta Day {day_count}: {new_sma50_1y_delta_ratio_stocks}")
                    sma50_1y_delta_ratio_positions, sma50_1y_delta_ratio_cash, current_sma50_1y_delta_ratio_stocks, rebalance_costs, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="SMA50 / 6M Delta",
                        current_stocks=current_sma50_1y_delta_ratio_stocks,
                        new_stocks=new_sma50_1y_delta_ratio_stocks,
                        positions=sma50_1y_delta_ratio_positions,
                        cash=sma50_1y_delta_ratio_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not current_sma50_1y_delta_ratio_stocks
                    )
                    strategies_rebalanced_today['SMA50 / 6M Delta'] = rebalanced_flag
                    sma50_1y_delta_ratio_transaction_costs += rebalance_costs
                    sma50_1y_delta_ratio_last_rebalance_value = _mark_to_market_value(
                        sma50_1y_delta_ratio_positions, sma50_1y_delta_ratio_cash, ticker_data_grouped, current_date
                    )
                else:
                    print(f"   ❌ No SMA50 / 6M Delta stocks selected")

            except Exception as e:
                print(f"   ⚠️ SMA50 / 6M Delta strategy error: {e}")
        ...
        # DEEP RECOVERY: Rebalance to beaten-down stocks with reversal signals DAILY
        if config.ENABLE_DEEP_RECOVERY:
            try:
                print(f"   🔄 Deep Recovery Strategy: Scanning full universe on {current_date.strftime('%Y-%m-%d')}")

                def _fallback_deep_recovery():
                    from deep_recovery_strategy import select_deep_recovery_stocks
                ...
                new_deep_recovery_stocks = _resolve_parallel_strategy_selection(
                    "deep_recovery",
                    _fallback_deep_recovery,
                )

                if new_deep_recovery_stocks:
                    print(f"   📊 Deep Recovery Day {day_count}: {new_deep_recovery_stocks}")
                    deep_recovery_positions, deep_recovery_cash, current_deep_recovery_stocks, rebalance_costs, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="Deep Recovery",
                        current_stocks=current_deep_recovery_stocks,
                        new_stocks=new_deep_recovery_stocks,
                        positions=deep_recovery_positions,
                        cash=deep_recovery_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not current_deep_recovery_stocks
                    )
                    strategies_rebalanced_today['Deep Recovery'] = rebalanced_flag
                    deep_recovery_transaction_costs += rebalance_costs
                    deep_recovery_last_rebalance_value = _mark_to_market_value(
                        deep_recovery_positions, deep_recovery_cash, ticker_data_grouped, current_date
                    )
                else:
                    print(f"   ❌ No Deep Recovery stocks selected")

            except Exception as e:
                print(f"   ⚠️ Deep Recovery strategy error: {e}")
        ...
        if config.ENABLE_PRICE_CURVATURE_1Y_SLOPE:
            try:
                print(f"   🌀 Price Curvature 1Y Slope Strategy: Analyzing {len(initial_top_tickers)} tickers on {current_date.strftime('%Y-%m-%d')}")

                def _fallback_price_curvature_1y_slope():
                    from price_curvature_strategy import select_price_curvature_1y_slope_stocks
                    return select_price_curvature_1y_slope_stocks(
                        initial_top_tickers,
                        ticker_data_grouped,
                        current_date=current_date,
                        top_n=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                ...
                new_price_curvature_1y_slope_stocks = _resolve_parallel_strategy_selection(
                    "price_curvature_1y_slope",
                    _fallback_price_curvature_1y_slope,
                )

                if new_price_curvature_1y_slope_stocks:
                    print(f"   📊 Price Curvature 1Y Slope Day {day_count}: {new_price_curvature_1y_slope_stocks}")
                    price_curvature_1y_slope_positions, price_curvature_1y_slope_cash, current_price_curvature_1y_slope_stocks, rebalance_costs, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="Price Curvature 1Y Slope",
                        current_stocks=current_price_curvature_1y_slope_stocks,
                        new_stocks=new_price_curvature_1y_slope_stocks,
                        positions=price_curvature_1y_slope_positions,
                        cash=price_curvature_1y_slope_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not current_price_curvature_1y_slope_stocks,
                    )
                    strategies_rebalanced_today['Price Curvature 1Y Slope'] = rebalanced_flag
                    price_curvature_1y_slope_transaction_costs += rebalance_costs
                    price_curvature_1y_slope_last_rebalance_value = _mark_to_market_value(
                        price_curvature_1y_slope_positions, price_curvature_1y_slope_cash, ticker_data_grouped, current_date
                    )
                else:
                    print("   ❌ No Price Curvature 1Y Slope stocks selected")

            except Exception as e:
                print(f"   ⚠️ Price Curvature 1Y Slope strategy error: {e}")
        ...
        if config.ENABLE_FORESIGHT_MIMIC:
            try:
                print(f"   🔮 Foresight Mimic Accel Strategy: Analyzing {len(initial_top_tickers)} tickers on {current_date.strftime('%Y-%m-%d')}")

                def _fallback_foresight_mimic():
                    from new_strategies import select_foresight_mimic_stocks
                ...
                new_foresight_mimic_stocks = _resolve_parallel_strategy_selection(
                    "foresight_mimic",
                    _fallback_foresight_mimic,
                )

                if new_foresight_mimic_stocks:
                    print(f"   📊 Foresight Mimic Accel Day {day_count}: {new_foresight_mimic_stocks}")
                    foresight_mimic_positions, foresight_mimic_cash, current_foresight_mimic_stocks, rebalance_costs, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="Foresight Mimic Accel",
                        current_stocks=current_foresight_mimic_stocks,
                        new_stocks=new_foresight_mimic_stocks,
                        positions=foresight_mimic_positions,
                        cash=foresight_mimic_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not current_foresight_mimic_stocks,
                    )
                    strategies_rebalanced_today['Foresight Mimic Accel'] = rebalanced_flag
                    foresight_mimic_transaction_costs += rebalance_costs
                    foresight_mimic_last_rebalance_value = _mark_to_market_value(
                        foresight_mimic_positions, foresight_mimic_cash, ticker_data_grouped, current_date
                    )
                else:
                    print("   ❌ No Foresight Mimic Accel stocks selected")

            except Exception as e:
                print(f"   ⚠️ Foresight Mimic Accel strategy error: {e}")

        if config.ENABLE_BH_1Y_1M_RANK:
            try:
                print(f"   📈 BH 1Y / 1M Rank Strategy: Analyzing {len(initial_top_tickers)} tickers on {current_date.strftime('%Y-%m-%d')}")

                def _fallback_bh_1y_1m_rank():
                    from shared_strategies import select_bh_1y_1m_rank_stocks
                    return select_bh_1y_1m_rank_stocks(
                        initial_top_tickers,
                        ticker_data_grouped,
                        current_date=current_date,
                        top_n=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                ...
                new_bh_1y_1m_rank_stocks = _resolve_parallel_strategy_selection(
                    "bh_1y_1m_rank",
                    _fallback_bh_1y_1m_rank,
                )

                if new_bh_1y_1m_rank_stocks:
                    print(f"   📊 BH 1Y / 1M Rank Day {day_count}: {new_bh_1y_1m_rank_stocks}")
                    bh_1y_1m_rank_positions, bh_1y_1m_rank_cash, current_bh_1y_1m_rank_stocks, rebalance_costs, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="BH 1Y / 1M Rank",
                        current_stocks=current_bh_1y_1m_rank_stocks,
                        new_stocks=new_bh_1y_1m_rank_stocks,
                        positions=bh_1y_1m_rank_positions,
                        cash=bh_1y_1m_rank_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not current_bh_1y_1m_rank_stocks,
                    )
                    strategies_rebalanced_today['BH 1Y / 1M Rank'] = rebalanced_flag
                    bh_1y_1m_rank_transaction_costs += rebalance_costs
                    bh_1y_1m_rank_last_rebalance_value = _mark_to_market_value(
                        bh_1y_1m_rank_positions, bh_1y_1m_rank_cash, ticker_data_grouped, current_date
                    )
                else:
                    print("   ❌ No BH 1Y / 1M Rank stocks selected")

            except Exception as e:
                print(f"   ⚠️ BH 1Y / 1M Rank strategy error: {e}")

        if config.ENABLE_BH_1Y_SMA200:
            try:
                print(f"   📈 BH 1Y SMA200 Strategy: Analyzing {len(initial_top_tickers)} tickers on {current_date.strftime('%Y-%m-%d')}")

                def _fallback_bh_1y_sma200():
                    from shared_strategies import select_bh_1y_sma200_stocks
                    return select_bh_1y_sma200_stocks(
                        initial_top_tickers,
                        ticker_data_grouped,
                        current_date=current_date,
                        top_n=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                ...
                new_bh_1y_sma200_stocks = _resolve_parallel_strategy_selection(
                    "bh_1y_sma200",
                    _fallback_bh_1y_sma200,
                )

                if new_bh_1y_sma200_stocks:
                    print(f"   📊 BH 1Y SMA200 Day {day_count}: {new_bh_1y_sma200_stocks}")
                    bh_1y_sma200_positions, bh_1y_sma200_cash, current_bh_1y_sma200_stocks, rebalance_costs, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="BH 1Y SMA200",
                        current_stocks=current_bh_1y_sma200_stocks,
                        new_stocks=new_bh_1y_sma200_stocks,
                        positions=bh_1y_sma200_positions,
                        cash=bh_1y_sma200_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not current_bh_1y_sma200_stocks,
                    )
                    strategies_rebalanced_today['BH 1Y SMA200'] = rebalanced_flag
                    bh_1y_sma200_transaction_costs += rebalance_costs
                    bh_1y_sma200_last_rebalance_value = _mark_to_market_value(
                        bh_1y_sma200_positions, bh_1y_sma200_cash, ticker_data_grouped, current_date
                    )
                else:
                    print("   ❌ No BH 1Y SMA200 stocks selected")

            except Exception as e:
                print(f"   ⚠️ BH 1Y SMA200 strategy error: {e}")

        if config.ENABLE_BH_1Y_FCF_RANK:
            try:
                print(f"   📈 BH 1Y / FCF Rank Strategy: Analyzing {len(initial_top_tickers)} tickers on {current_date.strftime('%Y-%m-%d')}")

                def _fallback_bh_1y_fcf_rank():
                    from shared_strategies import select_bh_1y_fcf_rank_stocks
                    return select_bh_1y_fcf_rank_stocks(
                        initial_top_tickers,
                        ticker_data_grouped,
                        current_date=current_date,
                        top_n=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                ...
                new_bh_1y_fcf_rank_stocks = _resolve_parallel_strategy_selection(
                    "bh_1y_fcf_rank",
                    _fallback_bh_1y_fcf_rank,
                )

                if new_bh_1y_fcf_rank_stocks:
                    print(f"   📊 BH 1Y / FCF Rank Day {day_count}: {new_bh_1y_fcf_rank_stocks}")
                    bh_1y_fcf_rank_positions, bh_1y_fcf_rank_cash, current_bh_1y_fcf_rank_stocks, rebalance_costs, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="BH 1Y / FCF Rank",
                        current_stocks=current_bh_1y_fcf_rank_stocks,
                        new_stocks=new_bh_1y_fcf_rank_stocks,
                        positions=bh_1y_fcf_rank_positions,
                        cash=bh_1y_fcf_rank_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not current_bh_1y_fcf_rank_stocks,
                    )
                    strategies_rebalanced_today['BH 1Y / FCF Rank'] = rebalanced_flag
                    bh_1y_fcf_rank_transaction_costs += rebalance_costs
                    bh_1y_fcf_rank_last_rebalance_value = _mark_to_market_value(
                        bh_1y_fcf_rank_positions, bh_1y_fcf_rank_cash, ticker_data_grouped, current_date
                    )
                else:
                    print("   ❌ No BH 1Y / FCF Rank stocks selected")

            except Exception as e:
                print(f"   ⚠️ BH 1Y / FCF Rank strategy error: {e}")

        if config.ENABLE_BH_1Y_6M_RANK:
            try:
                print(f"   📈 BH 1Y / 6M Rank Strategy: Analyzing {len(initial_top_tickers)} tickers on {current_date.strftime('%Y-%m-%d')}")

                def _fallback_bh_1y_6m_rank():
                    from shared_strategies import select_bh_1y_6m_rank_stocks
                    return select_bh_1y_6m_rank_stocks(
                        initial_top_tickers,
                        ticker_data_grouped,
                        current_date=current_date,
                        top_n=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                ...
                new_bh_1y_6m_rank_stocks = _resolve_parallel_strategy_selection(
                    "bh_1y_6m_rank",
                    _fallback_bh_1y_6m_rank,
                )

                if new_bh_1y_6m_rank_stocks:
                    print(f"   📊 BH 1Y / 6M Rank Day {day_count}: {new_bh_1y_6m_rank_stocks}")
                    bh_1y_6m_rank_positions, bh_1y_6m_rank_cash, current_bh_1y_6m_rank_stocks, rebalance_costs, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="BH 1Y / 6M Rank",
                        current_stocks=current_bh_1y_6m_rank_stocks,
                        new_stocks=new_bh_1y_6m_rank_stocks,
                        positions=bh_1y_6m_rank_positions,
                        cash=bh_1y_6m_rank_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not current_bh_1y_6m_rank_stocks,
                    )
                    strategies_rebalanced_today['BH 1Y / 6M Rank'] = rebalanced_flag
                    bh_1y_6m_rank_transaction_costs += rebalance_costs
                    bh_1y_6m_rank_last_rebalance_value = _mark_to_market_value(
                        bh_1y_6m_rank_positions, bh_1y_6m_rank_cash, ticker_data_grouped, current_date
                    )
                else:
                    print("   ❌ No BH 1Y / 6M Rank stocks selected")

            except Exception as e:
                print(f"   ⚠️ BH 1Y / 6M Rank strategy error: {e}")

        if config.ENABLE_BH_1Y_6M_BLEND:
            try:
                print(f"   📈 BH 1Y / 6M Blend Strategy: Analyzing {len(initial_top_tickers)} tickers on {current_date.strftime('%Y-%m-%d')}")

                def _fallback_bh_1y_6m_blend():
                    from shared_strategies import select_bh_1y_6m_blend_stocks
                    return select_bh_1y_6m_blend_stocks(
                        initial_top_tickers,
                        ticker_data_grouped,
                        current_date=current_date,
                        top_n=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                ...
                new_bh_1y_6m_blend_stocks = _resolve_parallel_strategy_selection(
                    "bh_1y_6m_blend",
                    _fallback_bh_1y_6m_blend,
                )

                if new_bh_1y_6m_blend_stocks:
                    print(f"   📊 BH 1Y / 6M Blend Day {day_count}: {new_bh_1y_6m_blend_stocks}")
                    bh_1y_6m_blend_positions, bh_1y_6m_blend_cash, current_bh_1y_6m_blend_stocks, rebalance_costs, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="BH 1Y / 6M Blend",
                        current_stocks=current_bh_1y_6m_blend_stocks,
                        new_stocks=new_bh_1y_6m_blend_stocks,
                        positions=bh_1y_6m_blend_positions,
                        cash=bh_1y_6m_blend_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not current_bh_1y_6m_blend_stocks,
                    )
                    strategies_rebalanced_today['BH 1Y / 6M Blend'] = rebalanced_flag
                    bh_1y_6m_blend_transaction_costs += rebalance_costs
                    bh_1y_6m_blend_last_rebalance_value = _mark_to_market_value(
                        bh_1y_6m_blend_positions, bh_1y_6m_blend_cash, ticker_data_grouped, current_date
                    )
                else:
                    print("   ❌ No BH 1Y / 6M Blend stocks selected")
        ...
        if config.ENABLE_EARLY_LEADER_ACCEL:
            try:
                print(f"   🚀 Early Leader Accel Strategy: Analyzing {len(initial_top_tickers)} tickers on {current_date.strftime('%Y-%m-%d')}")

                def _fallback_early_leader_accel():
                    from shared_strategies import select_early_leader_accel_stocks
                    return select_early_leader_accel_stocks(
                        initial_top_tickers,
                        ticker_data_grouped,
                        current_date=current_date,
                        top_n=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                ...
                new_early_leader_accel_stocks = _resolve_parallel_strategy_selection(
                    "early_leader_accel",
                    _fallback_early_leader_accel,
                )

                if new_early_leader_accel_stocks:
                    print(f"   📊 Early Leader Accel Day {day_count}: {new_early_leader_accel_stocks}")
                    early_leader_accel_positions, early_leader_accel_cash, current_early_leader_accel_stocks, rebalance_costs, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="Early Leader Accel",
                        current_stocks=current_early_leader_accel_stocks,
                        new_stocks=new_early_leader_accel_stocks,
                        positions=early_leader_accel_positions,
                        cash=early_leader_accel_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        force_rebalance=not current_early_leader_accel_stocks,
                    )
                    strategies_rebalanced_today['Early Leader Accel'] = rebalanced_flag
                    early_leader_accel_transaction_costs += rebalance_costs
                    early_leader_accel_last_rebalance_value = _mark_to_market_value(
                        early_leader_accel_positions, early_leader_accel_cash, ticker_data_grouped, current_date
                    )
                else:
                    print("   ❌ No Early Leader Accel stocks selected")

            except Exception as e:
                print(f"   ⚠️ Early Leader Accel strategy error: {e}")
        ...
        if config.ENABLE_QUALITY_MOM:
            try:
                ...
                if new_quality_momentum_stocks:
                    print(f"   📦 Quality+Mom portfolio before rebalance: {current_quality_momentum_stocks}")
                    ...
                    print(f"   📦 Quality+Mom portfolio after rebalance: {current_quality_momentum_stocks}")
            ...
            if should_act_ai_elite_monthly_shared:
                try:
                    ...
                    if ai_elite_models.get('_shared_base') is None:
                        ...
                        if new_stocks:
                            print(f"   📊 AI Elite Monthly Shared Day {day_count}: {new_stocks}")
                            ai_elite_monthly_shared_positions, ai_elite_monthly_shared_cash, current_ai_elite_monthly_shared_stocks, rc, rebalanced_flag = _smart_rebalance_portfolio(
                                strategy_name="AI Elite Mth Sh",
                                current_stocks=current_ai_elite_monthly_shared_stocks,
                                new_stocks=new_stocks,
                                positions=ai_elite_monthly_shared_positions,
                                cash=ai_elite_monthly_shared_cash,
                                ticker_data_grouped=ticker_data_grouped,
                                current_date=current_date,
                                transaction_cost=TRANSACTION_COST,
                                portfolio_size=PORTFOLIO_SIZE,
                                buffer_size=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                                force_rebalance=not ai_elite_monthly_shared_initialized)
                            strategies_rebalanced_today['AI Elite Mth Sh'] = rebalanced_flag
                            ai_elite_monthly_shared_transaction_costs += rc
                            ai_elite_monthly_shared_initialized = True
                            ai_elite_monthly_shared_last_month = current_date.month

                except Exception as e:
                    print(f"   ⚠️ AI Elite Monthly Shared error: {e}")
        ...
        # AI ELITE MARKET-UP SHARED STRATEGY (daily AI Elite shared model, only when market is up)
        if config.ENABLE_AI_ELITE_MARKET_UP:
            try:
                ...
                if new_ai_elite_market_up_stocks:
                    ...
                    print(f"   📊 AI Elite Market-Up Shared Day {day_count} - Before: {current_ai_elite_market_up_stocks}")
                    # Show candidates for rebalancing
                    print(f"   📊 AI Elite Market-Up Shared Day {day_count} - Candidates: {new_ai_elite_market_up_stocks}")
                    ...
                    print(f"   📊 AI Elite Market-Up Shared Day {day_count} - After: {current_ai_elite_market_up_stocks}")
                    strategies_rebalanced_today['AI Elite Up Sh'] = rebalanced_flag
                ...
                print(f"   ⚠️ AI Elite Market-Up Shared error: {e}")
        ...
        if config.ENABLE_AI_ELITE_ENSEMBLE:
            try:
                ...
                if new_ai_elite_ensemble_stocks:
                    print(f"   📊 AI Elite Ensemble Day {day_count}: {new_ai_elite_ensemble_stocks[:PORTFOLIO_SIZE]}")

                    ai_elite_ensemble_positions, ai_elite_ensemble_cash, current_ai_elite_ensemble_stocks, rc, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="AI Elite Ens",
                        current_stocks=current_ai_elite_ensemble_stocks,
                        new_stocks=new_ai_elite_ensemble_stocks,
                        positions=ai_elite_ensemble_positions,
                        cash=ai_elite_ensemble_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        buffer_size=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                        force_rebalance=not ai_elite_ensemble_initialized
                    )
                    strategies_rebalanced_today['AI Elite Ens'] = rebalanced_flag
                    ai_elite_ensemble_transaction_costs += rc
                    ai_elite_ensemble_initialized = True

            except Exception as e:
                print(f"   ⚠️ AI Elite Ensemble error: {e}")
        ...
        if config.ENABLE_AI_ELITE_RANK_ENSEMBLE:
            try:
                ...
                if new_ai_elite_rank_ensemble_stocks:
                    print(f"   📊 AI Elite Rank Ensemble Day {day_count}: {new_ai_elite_rank_ensemble_stocks[:PORTFOLIO_SIZE]}")

                    ai_elite_rank_ensemble_positions, ai_elite_rank_ensemble_cash, current_ai_elite_rank_ensemble_stocks, rc, rebalanced_flag = _smart_rebalance_portfolio(
                        strategy_name="AI Elite Rank",
                        current_stocks=current_ai_elite_rank_ensemble_stocks,
                        new_stocks=new_ai_elite_rank_ensemble_stocks,
                        positions=ai_elite_rank_ensemble_positions,
                        cash=ai_elite_rank_ensemble_cash,
                        ticker_data_grouped=ticker_data_grouped,
                        current_date=current_date,
                        transaction_cost=TRANSACTION_COST,
                        portfolio_size=PORTFOLIO_SIZE,
                        buffer_size=PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE,
                        force_rebalance=not ai_elite_rank_ensemble_initialized
                    )
                    strategies_rebalanced_today['AI Elite Rank'] = rebalanced_flag
                    ai_elite_rank_ensemble_transaction_costs += rc
                    ai_elite_rank_ensemble_initialized = True

            except Exception as e:
                print(f"   ⚠️ AI Elite Rank Ensemble error: {e}")
        ...
        if config.ENABLE_DYNAMIC_BH_1Y:
            dynamic_bh_portfolio_value = _update_portfolio_value_from_cache(
                ...
                dynamic_bh_cash,
                error_prefix="Error updating dynamic BH position",
                debug_message=f"   🔧 DEBUG: Dyn BH 1Y - iterating over {len(dynamic_bh_positions)} positions",
            )
        else:
            dynamic_bh_portfolio_value = dynamic_bh_cash
        ...
        if config.ENABLE_DYNAMIC_BH_1Y_VOL_FILTER:
            dynamic_bh_1y_vol_filter_portfolio_value = _update_portfolio_value_from_cache(
                dynamic_bh_1y_vol_filter_positions,
                dynamic_bh_1y_vol_filter_cash,
                error_prefix="Error updating dynamic BH 1Y+Vol position",
            )
        else:
            dynamic_bh_1y_vol_filter_portfolio_value = dynamic_bh_1y_vol_filter_cash
        ...
        if config.ENABLE_DYNAMIC_BH_1Y_TRAILING_STOP:
            dynamic_bh_1y_trailing_stop_portfolio_value = _update_portfolio_value_from_cache(
                dynamic_bh_1y_trailing_stop_positions,
                dynamic_bh_1y_trailing_stop_cash,
                error_prefix="Error updating dynamic BH 1Y+TS position",
            )
        else:
            dynamic_bh_1y_trailing_stop_portfolio_value = dynamic_bh_1y_trailing_stop_cash
        ...
        if config.ENABLE_DYNAMIC_BH_6M:
            dynamic_bh_6m_portfolio_value = _update_portfolio_value_from_cache(
                dynamic_bh_6m_positions,
                dynamic_bh_6m_cash,
            )
        else:
            dynamic_bh_6m_portfolio_value = dynamic_bh_6m_cash

        # Update DYNAMIC BH 3-MONTH portfolio value daily (skip if disabled)
        if config.ENABLE_DYNAMIC_BH_3M:
            dynamic_bh_3m_portfolio_value = _update_portfolio_value_from_cache(
                dynamic_bh_3m_positions,
                dynamic_bh_3m_cash,
                error_prefix="Error updating dynamic BH 3M position",
            )
        else:
            dynamic_bh_3m_portfolio_value = dynamic_bh_3m_cash
        ...
        if config.ENABLE_DYNAMIC_BH_1M:
            dynamic_bh_1m_portfolio_value = _update_portfolio_value_from_cache(
                dynamic_bh_1m_positions,
                dynamic_bh_1m_cash,
                error_prefix="Error updating dynamic BH 1M position",
            )
        else:
        ...
        if config.ENABLE_RISK_ADJ_MOM:
            risk_adj_mom_portfolio_value = _update_portfolio_value_from_cache(
                risk_adj_mom_positions,
                risk_adj_mom_cash,
                error_prefix="Error updating risk-adjusted momentum position",
            )
        else:
            risk_adj_mom_portfolio_value = risk_adj_mom_cash
        ...
        if config.ENABLE_RISK_ADJ_MOM_SENTIMENT:
            risk_adj_mom_sentiment_portfolio_value = _update_portfolio_value_from_cache(
                risk_adj_mom_sentiment_positions,
                risk_adj_mom_sentiment_cash,
                error_prefix="Error updating risk adj mom sentiment position",
            )
        ...
        # Update BH 1Y / 1M RANK portfolio value daily
        bh_1y_1m_rank_invested_value = 0.0
        if config.ENABLE_BH_1Y_1M_RANK:
            for ticker in list(bh_1y_1m_rank_positions.keys()):
                try:
                    ticker_data = ticker_data_grouped.get(ticker) if isinstance(ticker_data_grouped, dict) else None
                    if ticker_data is None and hasattr(ticker_data_grouped, 'get_group'):
                        try:
                    ...
                    if ticker_data is not None and not ticker_data.empty:
                        ...
                        if current_price is not None:
                            shares = bh_1y_1m_rank_positions[ticker]['shares']
                            position_value = shares * current_price
                            bh_1y_1m_rank_positions[ticker]['value'] = position_value
                            bh_1y_1m_rank_invested_value += position_value
                        else:
                            bh_1y_1m_rank_invested_value += bh_1y_1m_rank_positions[ticker].get('value', 0.0)
                    else:
                        bh_1y_1m_rank_invested_value += bh_1y_1m_rank_positions[ticker].get('value', 0.0)
                except Exception as e:
                    print(f"   ⚠️ Error updating BH 1Y / 1M Rank position for {ticker}: {e}")
                    bh_1y_1m_rank_invested_value += bh_1y_1m_rank_positions[ticker].get('value', 0.0)

        bh_1y_1m_rank_portfolio_value = bh_1y_1m_rank_invested_value + bh_1y_1m_rank_cash
        bh_1y_1m_rank_portfolio_history.append(bh_1y_1m_rank_portfolio_value)

        # Update BH 1Y SMA200 portfolio value daily
        bh_1y_sma200_invested_value = 0.0
        if config.ENABLE_BH_1Y_SMA200:
            for ticker in list(bh_1y_sma200_positions.keys()):
                try:
                    ticker_data = ticker_data_grouped.get(ticker) if isinstance(ticker_data_grouped, dict) else None
                    if ticker_data is None and hasattr(ticker_data_grouped, 'get_group'):
                        try:
                    ...
                    if ticker_data is not None and not ticker_data.empty:
                        current_price = _last_valid_close_up_to(ticker_data, current_date)
                        if current_price is not None:
                            shares = bh_1y_sma200_positions[ticker]['shares']
                            position_value = shares * current_price
                            bh_1y_sma200_positions[ticker]['value'] = position_value
                            bh_1y_sma200_invested_value += position_value
                        else:
                            bh_1y_sma200_invested_value += bh_1y_sma200_positions[ticker].get('value', 0.0)
                    else:
                        bh_1y_sma200_invested_value += bh_1y_sma200_positions[ticker].get('value', 0.0)
                except Exception as e:
                    print(f"   ⚠️ Error updating BH 1Y SMA200 position for {ticker}: {e}")
                    bh_1y_sma200_invested_value += bh_1y_sma200_positions[ticker].get('value', 0.0)

        bh_1y_sma200_portfolio_value = bh_1y_sma200_invested_value + bh_1y_sma200_cash
        bh_1y_sma200_portfolio_history.append(bh_1y_sma200_portfolio_value)

        # Update BH 1Y / FCF RANK portfolio value daily
        bh_1y_fcf_rank_invested_value = 0.0
        if config.ENABLE_BH_1Y_FCF_RANK:
            for ticker in list(bh_1y_fcf_rank_positions.keys()):
                try:
                    ticker_data = ticker_data_grouped.get(ticker) if isinstance(ticker_data_grouped, dict) else None
                    if ticker_data is None and hasattr(ticker_data_grouped, 'get_group'):
                        try:
                    ...
                    if ticker_data is not None and not ticker_data.empty:
                        current_price = _last_valid_close_up_to(ticker_data, current_date)
                        if current_price is not None:
                            shares = bh_1y_fcf_rank_positions[ticker]['shares']
                            position_value = shares * current_price
                            bh_1y_fcf_rank_positions[ticker]['value'] = position_value
                            bh_1y_fcf_rank_invested_value += position_value
                        else:
                            bh_1y_fcf_rank_invested_value += bh_1y_fcf_rank_positions[ticker].get('value', 0.0)
                    else:
                        bh_1y_fcf_rank_invested_value += bh_1y_fcf_rank_positions[ticker].get('value', 0.0)
                except Exception as e:
                    print(f"   ⚠️ Error updating BH 1Y / FCF Rank position for {ticker}: {e}")
                    bh_1y_fcf_rank_invested_value += bh_1y_fcf_rank_positions[ticker].get('value', 0.0)

        bh_1y_fcf_rank_portfolio_value = bh_1y_fcf_rank_invested_value + bh_1y_fcf_rank_cash
        bh_1y_fcf_rank_portfolio_history.append(bh_1y_fcf_rank_portfolio_value)

        # Update BH 1Y / 6M RANK portfolio value daily
        bh_1y_6m_rank_invested_value = 0.0
        if config.ENABLE_BH_1Y_6M_RANK:
            for ticker in list(bh_1y_6m_rank_positions.keys()):
                try:
                    ticker_data = ticker_data_grouped.get(ticker) if isinstance(ticker_data_grouped, dict) else None
                    if ticker_data is None and hasattr(ticker_data_grouped, 'get_group'):
                        try:
                    ...
                    if ticker_data is not None and not ticker_data.empty:
                        current_price = _last_valid_close_up_to(ticker_data, current_date)
                        if current_price is not None:
                            shares = bh_1y_6m_rank_positions[ticker]['shares']
                            position_value = shares * current_price
                            bh_1y_6m_rank_positions[ticker]['value'] = position_value
                            bh_1y_6m_rank_invested_value += position_value
                        else:
                            bh_1y_6m_rank_invested_value += bh_1y_6m_rank_positions[ticker].get('value', 0.0)
                    else:
                        bh_1y_6m_rank_invested_value += bh_1y_6m_rank_positions[ticker].get('value', 0.0)
                except Exception as e:
                    print(f"   ⚠️ Error updating BH 1Y / 6M Rank position for {ticker}: {e}")
                    bh_1y_6m_rank_invested_value += bh_1y_6m_rank_positions[ticker].get('value', 0.0)

        bh_1y_6m_rank_portfolio_value = bh_1y_6m_rank_invested_value + bh_1y_6m_rank_cash
        bh_1y_6m_rank_portfolio_history.append(bh_1y_6m_rank_portfolio_value)

        # Update BH 1Y / 6M BLEND portfolio value daily
        bh_1y_6m_blend_invested_value = 0.0
        if config.ENABLE_BH_1Y_6M_BLEND:
            for ticker in list(bh_1y_6m_blend_positions.keys()):
                try:
                    ticker_data = ticker_data_grouped.get(ticker) if isinstance(ticker_data_grouped, dict) else None
                    if ticker_data is None and hasattr(ticker_data_grouped, 'get_group'):
                        try:
                    ...
                    if ticker_data is not None and not ticker_data.empty:
                        current_price = _last_valid_close_up_to(ticker_data, current_date)
                        if current_price is not None:
                            shares = bh_1y_6m_blend_positions[ticker]['shares']
                            position_value = shares * current_price
                            bh_1y_6m_blend_positions[ticker]['value'] = position_value
                            bh_1y_6m_blend_invested_value += position_value
                        else:
                            bh_1y_6m_blend_invested_value += bh_1y_6m_blend_positions[ticker].get('value', 0.0)
                    else:
                        bh_1y_6m_blend_invested_value += bh_1y_6m_blend_positions[ticker].get('value', 0.0)
                except Exception as e:
                    print(f"   ⚠️ Error updating BH 1Y / 6M Blend position for {ticker}: {e}")
                    bh_1y_6m_blend_invested_value += bh_1y_6m_blend_positions[ticker].get('value', 0.0)
        ...
        if config.ENABLE_EARLY_LEADER_ACCEL:
            for ticker in list(early_leader_accel_positions.keys()):
                try:
                    ticker_data = ticker_data_grouped.get(ticker) if isinstance(ticker_data_grouped, dict) else None
                    if ticker_data is None and hasattr(ticker_data_grouped, 'get_group'):
                        try:
                    ...
                    if ticker_data is not None and not ticker_data.empty:
                        current_price = _last_valid_close_up_to(ticker_data, current_date)
                        if current_price is not None:
                            shares = early_leader_accel_positions[ticker]['shares']
                            position_value = shares * current_price
                            early_leader_accel_positions[ticker]['value'] = position_value
                            early_leader_accel_invested_value += position_value
                        else:
                            early_leader_accel_invested_value += early_leader_accel_positions[ticker].get('value', 0.0)
                    else:
                        early_leader_accel_invested_value += early_leader_accel_positions[ticker].get('value', 0.0)
                except Exception as e:
                    print(f"   ⚠️ Error updating Early Leader Accel position for {ticker}: {e}")
                    early_leader_accel_invested_value += early_leader_accel_positions[ticker].get('value', 0.0)

        early_leader_accel_portfolio_value = early_leader_accel_invested_value + early_leader_accel_cash
        early_leader_accel_portfolio_history.append(early_leader_accel_portfolio_value)
        ...
        # Update STATIC PRICE CURVATURE portfolio value daily
        static_price_curvature_invested_value = 0.0
        if config.ENABLE_STATIC_PRICE_CURVATURE:
            for ticker in list(static_price_curvature_positions.keys()):
                try:
                    ticker_data = ticker_data_grouped.get(ticker) if isinstance(ticker_data_grouped, dict) else None
                    if ticker_data is None and hasattr(ticker_data_grouped, 'get_group'):
                        try:
                    ...
                    if ticker_data is not None and not ticker_data.empty:
                        current_price = _last_valid_close_up_to(ticker_data, current_date)
                        if current_price is not None:
                            shares = static_price_curvature_positions[ticker]['shares']
                            position_value = shares * current_price
                            static_price_curvature_positions[ticker]['value'] = position_value
                            static_price_curvature_invested_value += position_value
                        else:
                            static_price_curvature_invested_value += static_price_curvature_positions[ticker].get('value', 0.0)
                    else:
                        static_price_curvature_invested_value += static_price_curvature_positions[ticker].get('value', 0.0)
                except Exception as e:
                    print(f"   ⚠️ Error updating Static Price Curvature position for {ticker}: {e}")
                    static_price_curvature_invested_value += static_price_curvature_positions[ticker].get('value', 0.0)

        static_price_curvature_portfolio_value = static_price_curvature_invested_value + static_price_curvature_cash
        static_price_curvature_portfolio_history.append(static_price_curvature_portfolio_value)

        # Update PRICE CURVATURE 1Y SLOPE portfolio value daily
        price_curvature_1y_slope_invested_value = 0.0
        if config.ENABLE_PRICE_CURVATURE_1Y_SLOPE:
            for ticker in list(price_curvature_1y_slope_positions.keys()):
                try:
                    ticker_data = ticker_data_grouped.get(ticker) if isinstance(ticker_data_grouped, dict) else None
                    if ticker_data is None and hasattr(ticker_data_grouped, 'get_group'):
                        try:
                    ...
                    if ticker_data is not None and not ticker_data.empty:
                        current_price = _last_valid_close_up_to(ticker_data, current_date)
                        if current_price is not None:
                            shares = price_curvature_1y_slope_positions[ticker]['shares']
                            position_value = shares * current_price
                            price_curvature_1y_slope_positions[ticker]['value'] = position_value
                            price_curvature_1y_slope_invested_value += position_value
                        else:
                            price_curvature_1y_slope_invested_value += price_curvature_1y_slope_positions[ticker].get('value', 0.0)
                    else:
                        price_curvature_1y_slope_invested_value += price_curvature_1y_slope_positions[ticker].get('value', 0.0)
                except Exception as e:
                    print(f"   ⚠️ Error updating Price Curvature 1Y Slope position for {ticker}: {e}")
                    price_curvature_1y_slope_invested_value += price_curvature_1y_slope_positions[ticker].get('value', 0.0)

        price_curvature_1y_slope_portfolio_value = price_curvature_1y_slope_invested_value + price_curvature_1y_slope_cash
        price_curvature_1y_slope_portfolio_history.append(price_curvature_1y_slope_portfolio_value)

        # Update STATIC PRICE CURVATURE 1Y SLOPE portfolio value daily
        static_price_curvature_1y_slope_invested_value = 0.0
        if config.ENABLE_STATIC_PRICE_CURVATURE_1Y_SLOPE:
            for ticker in list(static_price_curvature_1y_slope_positions.keys()):
                try:
                    ticker_data = ticker_data_grouped.get(ticker) if isinstance(ticker_data_grouped, dict) else None
                    if ticker_data is None and hasattr(ticker_data_grouped, 'get_group'):
                        try:
                    ...
                    if ticker_data is not None and not ticker_data.empty:
                        current_price = _last_valid_close_up_to(ticker_data, current_date)
                        if current_price is not None:
                            shares = static_price_curvature_1y_slope_positions[ticker]['shares']
                            position_value = shares * current_price
                            static_price_curvature_1y_slope_positions[ticker]['value'] = position_value
                            static_price_curvature_1y_slope_invested_value += position_value
                        else:
                            static_price_curvature_1y_slope_invested_value += static_price_curvature_1y_slope_positions[ticker].get('value', 0.0)
                    else:
                        static_price_curvature_1y_slope_invested_value += static_price_curvature_1y_slope_positions[ticker].get('value', 0.0)
                except Exception as e:
                    print(f"   ⚠️ Error updating Static Price Curvature 1Y Slope position for {ticker}: {e}")
                    static_price_curvature_1y_slope_invested_value += static_price_curvature_1y_slope_positions[ticker].get('value', 0.0)

        static_price_curvature_1y_slope_portfolio_value = static_price_curvature_1y_slope_invested_value + static_price_curvature_1y_slope_cash
        static_price_curvature_1y_slope_portfolio_history.append(static_price_curvature_1y_slope_portfolio_value)
        ...
        # Update FORESIGHT MIMIC portfolio value daily
        foresight_mimic_invested_value = 0.0
        if config.ENABLE_FORESIGHT_MIMIC:
            for ticker in list(foresight_mimic_positions.keys()):
                try:
                    ticker_data = ticker_data_grouped.get(ticker) if isinstance(ticker_data_grouped, dict) else None
                    if ticker_data is not None and not ticker_data.empty:
                        current_price = _last_valid_close_up_to(ticker_data, current_date)
                        if current_price is not None:
                            shares = foresight_mimic_positions[ticker]['shares']
                            position_value = shares * current_price
                            foresight_mimic_positions[ticker]['value'] = position_value
                            foresight_mimic_invested_value += position_value
                        else:
                            foresight_mimic_invested_value += foresight_mimic_positions[ticker].get('value', 0.0)
                    else:
                        foresight_mimic_invested_value += foresight_mimic_positions[ticker].get('value', 0.0)
                except Exception as e:
                    print(f"   ⚠️ Error updating Foresight Mimic position for {ticker}: {e}")
                    foresight_mimic_invested_value += foresight_mimic_positions[ticker].get('value', 0.0)

        foresight_mimic_portfolio_value = foresight_mimic_invested_value + foresight_mimic_cash
        foresight_mimic_portfolio_history.append(foresight_mimic_portfolio_value)
        ...
        # Update SMA50 / 6M DELTA RATIO portfolio value daily
        if config.ENABLE_SMA50_1Y_DELTA_RATIO:
            sma50_1y_delta_ratio_invested_value = 0.0
            for ticker in list(sma50_1y_delta_ratio_positions.keys()):
                try:
                    ticker_df = ticker_data_grouped.get(ticker)
                    if ticker_df is not None:
                        current_price = _last_valid_close_up_to(ticker_df, current_date)
                        if current_price is not None:
                            shares = sma50_1y_delta_ratio_positions[ticker]['shares']
                            position_value = shares * current_price
                            sma50_1y_delta_ratio_positions[ticker]['value'] = position_value
                            sma50_1y_delta_ratio_invested_value += position_value
                        else:
                            sma50_1y_delta_ratio_invested_value += sma50_1y_delta_ratio_positions[ticker].get('value', 0.0)
                    else:
                        sma50_1y_delta_ratio_invested_value += sma50_1y_delta_ratio_positions[ticker].get('value', 0.0)
                except Exception as e:
                    print(f"   ⚠️ Error updating SMA50 / 6M Delta position for {ticker}: {e}")
                    sma50_1y_delta_ratio_invested_value += sma50_1y_delta_ratio_positions[ticker].get('value', 0.0)

            sma50_1y_delta_ratio_portfolio_value = sma50_1y_delta_ratio_invested_value + sma50_1y_delta_ratio_cash
            sma50_1y_delta_ratio_portfolio_history.append(sma50_1y_delta_ratio_portfolio_value)
        ...
        # Update DEEP RECOVERY portfolio value daily
        deep_recovery_invested_value = 0.0
        for ticker in list(deep_recovery_positions.keys()):
            try:
                ticker_df = ticker_data_grouped.get(ticker)
                if ticker_df is not None:
                    current_price = _last_valid_close_up_to(ticker_df, current_date)
                    if current_price is not None:
                        shares = deep_recovery_positions[ticker]['shares']
                        position_value = shares * current_price
                        deep_recovery_positions[ticker]['value'] = position_value
                        deep_recovery_invested_value += position_value
                    else:
                        deep_recovery_invested_value += deep_recovery_positions[ticker].get('value', 0.0)
                else:
                    deep_recovery_invested_value += deep_recovery_positions[ticker].get('value', 0.0)
            except Exception as e:
                print(f"   ⚠️ Error updating deep recovery position for {ticker}: {e}")
                deep_recovery_invested_value += deep_recovery_positions[ticker].get('value', 0.0)

        deep_recovery_portfolio_value = deep_recovery_invested_value + deep_recovery_cash
        deep_recovery_portfolio_history.append(deep_recovery_portfolio_value)
        ...
        if config.ENABLE_MULTI_TIMEFRAME_ENSEMBLE:
            multi_tf_ensemble_portfolio_value = _update_portfolio_value_from_cache(
                multi_tf_ensemble_positions,
                multi_tf_ensemble_cash,
                error_prefix="Error updating multi-timeframe ensemble position",
            )
        else:
            multi_tf_ensemble_portfolio_value = multi_tf_ensemble_cash
        ...
        if config.ENABLE_MULTI_TIMEFRAME_INTRADAY_ENSEMBLE:
            multi_tf_intraday_ensemble_portfolio_value = _update_portfolio_value_from_cache(
                multi_tf_intraday_ensemble_positions,
                multi_tf_intraday_ensemble_cash,
                error_prefix="Error updating multi-timeframe intraday ensemble position",
            )
        else:
            multi_tf_intraday_ensemble_portfolio_value = multi_tf_intraday_ensemble_cash
        ...
        if config.ENABLE_AI_ELITE_MONTHLY_SHARED:
            for ticker in list(ai_elite_monthly_shared_positions.keys()):
                try:
                    ticker_df = ticker_data_grouped.get(ticker)
                    if ticker_df is not None:
                        current_price = _last_valid_close_up_to(ticker_df, current_date)
                        if current_price is not None:
                            shares = ai_elite_monthly_shared_positions[ticker]['shares']
                            position_value = shares * current_price
                            ai_elite_monthly_shared_positions[ticker]['value'] = position_value
                            ai_elite_monthly_shared_invested_value += position_value
                        else:
                            ai_elite_monthly_shared_invested_value += ai_elite_monthly_shared_positions[ticker].get('value', 0.0)
                    else:
                        ai_elite_monthly_shared_invested_value += ai_elite_monthly_shared_positions[ticker].get('value', 0.0)
                except Exception:
                    ai_elite_monthly_shared_invested_value += ai_elite_monthly_shared_positions[ticker].get('value', 0.0)
        ai_elite_monthly_shared_portfolio_value = ai_elite_monthly_shared_invested_value + ai_elite_monthly_shared_cash
        ai_elite_monthly_shared_portfolio_history.append(ai_elite_monthly_shared_portfolio_value)
        ...
        # Update AI ELITE ENSEMBLE portfolio value daily
        ai_elite_ensemble_invested_value = 0.0
        if config.ENABLE_AI_ELITE_ENSEMBLE:
            for ticker in list(ai_elite_ensemble_positions.keys()):
                try:
                    ticker_df = ticker_data_grouped.get(ticker)
                    if ticker_df is not None:
                        current_price = _last_valid_close_up_to(ticker_df, current_date)
                        if current_price is not None:
                            shares = ai_elite_ensemble_positions[ticker]['shares']
                            position_value = shares * current_price
                            ai_elite_ensemble_positions[ticker]['value'] = position_value
                            ai_elite_ensemble_invested_value += position_value
                        else:
                            ai_elite_ensemble_invested_value += ai_elite_ensemble_positions[ticker].get('value', 0.0)
                    else:
                        ai_elite_ensemble_invested_value += ai_elite_ensemble_positions[ticker].get('value', 0.0)
                except Exception:
                    ai_elite_ensemble_invested_value += ai_elite_ensemble_positions[ticker].get('value', 0.0)
        ai_elite_ensemble_portfolio_value = ai_elite_ensemble_invested_value + ai_elite_ensemble_cash
        ai_elite_ensemble_portfolio_history.append(ai_elite_ensemble_portfolio_value)

        # Update AI ELITE RANK ENSEMBLE portfolio value daily
        ai_elite_rank_ensemble_invested_value = 0.0
        if config.ENABLE_AI_ELITE_RANK_ENSEMBLE:
            for ticker in list(ai_elite_rank_ensemble_positions.keys()):
                try:
                    ticker_df = ticker_data_grouped.get(ticker)
                    if ticker_df is not None:
                        current_price = _last_valid_close_up_to(ticker_df, current_date)
                        if current_price is not None:
                            shares = ai_elite_rank_ensemble_positions[ticker]['shares']
                            position_value = shares * current_price
                            ai_elite_rank_ensemble_positions[ticker]['value'] = position_value
                            ai_elite_rank_ensemble_invested_value += position_value
                        else:
                            ai_elite_rank_ensemble_invested_value += ai_elite_rank_ensemble_positions[ticker].get('value', 0.0)
                    else:
                        ai_elite_rank_ensemble_invested_value += ai_elite_rank_ensemble_positions[ticker].get('value', 0.0)
                except Exception:
                    ai_elite_rank_ensemble_invested_value += ai_elite_rank_ensemble_positions[ticker].get('value', 0.0)
        ai_elite_rank_ensemble_portfolio_value = ai_elite_rank_ensemble_invested_value + ai_elite_rank_ensemble_cash
        ai_elite_rank_ensemble_portfolio_history.append(ai_elite_rank_ensemble_portfolio_value)
        ...
        def _update_meta_portfolio(positions, cash):
            ...
            return _update_portfolio_value_from_cache(positions, cash)
        ...
        if config.ENABLE_MEAN_REVERSION:
            mean_reversion_portfolio_value = _update_portfolio_value_from_cache(mean_reversion_positions, mean_reversion_cash)
        else:
            mean_reversion_portfolio_value = mean_reversion_cash
        ...
        if config.ENABLE_QUALITY_MOM:
            quality_momentum_portfolio_value = _update_portfolio_value_from_cache(quality_momentum_positions, quality_momentum_cash)
        else:
            quality_momentum_portfolio_value = quality_momentum_cash
        ...
        if config.ENABLE_VOLATILITY_ADJ_MOM:
            volatility_adj_mom_portfolio_value = _update_portfolio_value_from_cache(volatility_adj_mom_positions, volatility_adj_mom_cash)
        else:
            volatility_adj_mom_portfolio_value = volatility_adj_mom_cash
        ...
        if config.ENABLE_INVERSE_ETF_HEDGE:
            inverse_etf_hedge_portfolio_value = _update_portfolio_value_from_cache(inverse_etf_hedge_positions, inverse_etf_hedge_cash)
        ...
        if config.ENABLE_ANALYST_RECOMMENDATION:
            analyst_rec_portfolio_value = _update_portfolio_value_from_cache(analyst_rec_positions, analyst_rec_cash)
        ...
        if config.ENABLE_STATIC_BH:
            static_bh_1y_portfolio_value = _update_portfolio_value_from_cache(
                static_bh_1y_positions,
                static_bh_1y_cash,
                error_prefix="Error updating Static BH 1Y position",
            )
        else:
            static_bh_1y_portfolio_value = static_bh_1y_cash
        ...
        # 1. Volatility-triggered
        if config.ENABLE_STATIC_BH_1Y_VOLATILITY:
            static_bh_1y_vol_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_vol_positions, static_bh_1y_vol_cash)
            static_bh_1y_vol_portfolio_history.append(static_bh_1y_vol_portfolio_value)

        # 2. Performance-triggered
        if config.ENABLE_STATIC_BH_1Y_PERFORMANCE:
            static_bh_1y_perf_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_perf_positions, static_bh_1y_perf_cash)
            static_bh_1y_perf_portfolio_history.append(static_bh_1y_perf_portfolio_value)

        if config.ENABLE_STATIC_BH_6M_PERFORMANCE:
            static_bh_6m_perf_portfolio_value = _update_portfolio_value_from_cache(static_bh_6m_perf_positions, static_bh_6m_perf_cash)
            static_bh_6m_perf_portfolio_history.append(static_bh_6m_perf_portfolio_value)

        if config.ENABLE_STATIC_BH_9M_PERFORMANCE:
            static_bh_9m_perf_portfolio_value = _update_portfolio_value_from_cache(static_bh_9m_perf_positions, static_bh_9m_perf_cash)
            static_bh_9m_perf_portfolio_history.append(static_bh_9m_perf_portfolio_value)

        # 3. Momentum-triggered
        if config.ENABLE_STATIC_BH_1Y_MOMENTUM:
            static_bh_1y_mom_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_mom_positions, static_bh_1y_mom_cash)
            static_bh_1y_mom_portfolio_history.append(static_bh_1y_mom_portfolio_value)

        # 4. ATR-triggered
        if config.ENABLE_STATIC_BH_1Y_ATR:
            static_bh_1y_atr_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_atr_positions, static_bh_1y_atr_cash)
            static_bh_1y_atr_portfolio_history.append(static_bh_1y_atr_portfolio_value)

        # 5. Hybrid
        if config.ENABLE_STATIC_BH_1Y_HYBRID:
            static_bh_1y_hybrid_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_hybrid_positions, static_bh_1y_hybrid_cash)
            static_bh_1y_hybrid_portfolio_history.append(static_bh_1y_hybrid_portfolio_value)

        # 6. Volume Filter
        if config.ENABLE_STATIC_BH_1Y_VOLUME_FILTER:
            static_bh_1y_volume_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_volume_positions, static_bh_1y_volume_cash)
            static_bh_1y_volume_portfolio_history.append(static_bh_1y_volume_portfolio_value)

        # 7. Sector Rotation
        if config.ENABLE_STATIC_BH_1Y_SECTOR_ROTATION:
            static_bh_1y_sector_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_sector_positions, static_bh_1y_sector_cash)
            static_bh_1y_sector_portfolio_history.append(static_bh_1y_sector_portfolio_value)

        # 8. Performance Threshold
        if config.ENABLE_STATIC_BH_1Y_PERFORMANCE_THRESHOLD:
            static_bh_1y_perf_threshold_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_perf_threshold_positions, static_bh_1y_perf_threshold_cash)
            static_bh_1y_perf_threshold_portfolio_history.append(static_bh_1y_perf_threshold_portfolio_value)

        # 9. Market Regime
        if config.ENABLE_STATIC_BH_1Y_MARKET_REGIME:
            static_bh_1y_market_regime_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_market_regime_positions, static_bh_1y_market_regime_cash)
            static_bh_1y_market_regime_portfolio_history.append(static_bh_1y_market_regime_portfolio_value)

        # 10. Momentum Persistence
        if config.ENABLE_STATIC_BH_1Y_MOMENTUM_PERSIST:
            static_bh_1y_mom_persist_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_mom_persist_positions, static_bh_1y_mom_persist_cash)
            static_bh_1y_mom_persist_portfolio_history.append(static_bh_1y_mom_persist_portfolio_value)

        # 11. Overlap-Based
        if config.ENABLE_STATIC_BH_1Y_OVERLAP:
            static_bh_1y_overlap_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_overlap_positions, static_bh_1y_overlap_cash)
            static_bh_1y_overlap_portfolio_history.append(static_bh_1y_overlap_portfolio_value)

        # 12. Rank Drift
        if config.ENABLE_STATIC_BH_1Y_RANK_DRIFT:
            static_bh_1y_rank_drift_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_rank_drift_positions, static_bh_1y_rank_drift_cash)
            static_bh_1y_rank_drift_portfolio_history.append(static_bh_1y_rank_drift_portfolio_value)

        # 13. Drawdown Trigger
        if config.ENABLE_STATIC_BH_1Y_DRAWDOWN:
            static_bh_1y_drawdown_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_drawdown_positions, static_bh_1y_drawdown_cash)
        ...
        # 14. Smart Monthly
        if config.ENABLE_STATIC_BH_1Y_SMART_MONTHLY:
            static_bh_1y_smart_monthly_portfolio_value = _update_portfolio_value_from_cache(static_bh_1y_smart_monthly_positions, static_bh_1y_smart_monthly_cash)
        ...
        if config.ENABLE_STATIC_BH_6M:
            static_bh_6m_portfolio_value = _update_portfolio_value_from_cache(
                static_bh_6m_positions,
                static_bh_6m_cash,
                error_prefix="Error updating Static BH 6M position",
            )
        else:
            static_bh_6m_portfolio_value = static_bh_6m_cash
        ...
        if config.ENABLE_STATIC_BH:
            static_bh_3m_portfolio_value = _update_portfolio_value_from_cache(
                static_bh_3m_positions,
                static_bh_3m_cash,
                error_prefix="Error updating Static BH 3M position",
            )
        else:
            static_bh_3m_portfolio_value = static_bh_3m_cash
        ...
        if config.ENABLE_STATIC_BH:
            static_bh_1m_portfolio_value = _update_portfolio_value_from_cache(
                static_bh_1m_positions,
                static_bh_1m_cash,
                error_prefix="Error updating Static BH 1M position",
            )
        else:
            static_bh_1m_portfolio_value = static_bh_1m_cash
        ...
        for monthly_var in [
            ...
            if enabled:
                invested = _update_portfolio_value_from_cache(positions_dict, 0.0)
        ...
        if config.ENABLE_VOTING_ENSEMBLE or config.ENABLE_VOTING_ENSEMBLE_AI_REBALANCE:
            try:
                ...
                new_voting_ensemble_stocks, voting_sources, ranked_votes, vote_details = _time_strategy_phase_call(
                    ...
                    excluded_strategy_keys=['voting_ensemble', 'voting_ensemble_ai_rebalance'],
                )
                ...
                if voting_sources:
                    ...
                    if config.ENABLE_VOTING_ENSEMBLE_AI_REBALANCE:
                        (
                            voting_ensemble_ai_rebalance_positions,
                            voting_ensemble_ai_rebalance_cash,
                            current_voting_ensemble_ai_rebalance_stocks,
                            rebalance_costs,
                            rebalanced_flag,
                            ai_rebalance_decision_logs,
                            ai_rebalance_ranked_candidates,
                        ) = _ai_rebalance_portfolio(
                            strategy_name="Voting Ens AI Reb",
                            current_stocks=current_voting_ensemble_ai_rebalance_stocks,
                            ranked_candidates=new_voting_ensemble_stocks,
                            positions=voting_ensemble_ai_rebalance_positions,
                            cash=voting_ensemble_ai_rebalance_cash,
                            ticker_data_grouped=ticker_data_grouped,
                            current_date=current_date,
                            transaction_cost=TRANSACTION_COST,
                            model_bundle=voting_ensemble_ai_rebalance_model,
                            portfolio_size=PORTFOLIO_SIZE,
                        ...
                        if ai_rebalance_decision_logs:
                            print("   🤖 Voting Ens AI Reb decisions:")
                            for held_ticker, candidate_ticker, predicted_edge, action in ai_rebalance_decision_logs[:10]:
                                print(
                                    f"      {held_ticker} vs {candidate_ticker}: "
                                    f"{action} (predicted edge {predicted_edge:+.3f})"
                                )
                        if ai_rebalance_ranked_candidates and ai_rebalance_ranked_candidates != new_voting_ensemble_stocks:
                            print(f"   🤖 Voting Ens AI Reb reordered candidates: {ai_rebalance_ranked_candidates[:PORTFOLIO_SIZE + PORTFOLIO_BUFFER_SIZE]}")
                else:
            ...
            if config.ENABLE_VOTING_ENSEMBLE_AI_REBALANCE:
                voting_ensemble_ai_rebalance_invested_value = 0.0
                for ticker in list(voting_ensemble_ai_rebalance_positions.keys()):
                    try:
                        ticker_df = ticker_data_grouped.get(ticker)
                        if ticker_df is not None:
                            current_price = _last_valid_close_up_to(ticker_df, current_date)
                            if current_price is not None:
                                shares = voting_ensemble_ai_rebalance_positions[ticker]['shares']
                                position_value = shares * current_price
                                voting_ensemble_ai_rebalance_positions[ticker]['value'] = position_value
                                voting_ensemble_ai_rebalance_invested_value += position_value
                            else:
                                voting_ensemble_ai_rebalance_invested_value += voting_ensemble_ai_rebalance_positions[ticker].get('value', 0.0)
                        else:
                            voting_ensemble_ai_rebalance_invested_value += voting_ensemble_ai_rebalance_positions[ticker].get('value', 0.0)
                    except Exception as e:
                        print(f"   ⚠️ Error updating voting AI rebalance position for {ticker}: {e}")
                        voting_ensemble_ai_rebalance_invested_value += voting_ensemble_ai_rebalance_positions[ticker].get('value', 0.0)

                voting_ensemble_ai_rebalance_portfolio_value = (
                    voting_ensemble_ai_rebalance_invested_value + voting_ensemble_ai_rebalance_cash
                )
                voting_ensemble_ai_rebalance_portfolio_history.append(voting_ensemble_ai_rebalance_portfolio_value)

            if config.ENABLE_VOTING_ENSEMBLE and config.ENABLE_VOTING_ENSEMBLE_AI_REBALANCE:
                value_diff = voting_ensemble_ai_rebalance_portfolio_value - voting_ensemble_portfolio_value
                cost_diff = voting_ensemble_ai_rebalance_transaction_costs - voting_ensemble_transaction_costs
                print(
                    f"   📈 Voting comparison: AI Reb "
                    f"${voting_ensemble_ai_rebalance_portfolio_value:,.0f} vs "
                    f"Base ${voting_ensemble_portfolio_value:,.0f} "
                    f"(diff {value_diff:+,.0f}, costs diff {cost_diff:+,.0f})"
                )
        ...
        if daily_strat_data:
            ...
            summary_rank_width = 5
            summary_name_width = 32
            summary_value_width = 13
            summary_return_width = 9
            summary_stddev_width = 8
            ...
            summary_row_fmt = (
                ...
                f"{{value:>{summary_value_width}}} "
                f"{{ret:>{summary_return_width}}} "
            ...
            summary_line_width = (
                ...
                + summary_name_width
                + summary_value_width
                + summary_return_width
                + summary_stddev_width
        ...
        strategy_histories = [
            ...
            ("SMA50 / 6M Delta",   sma50_1y_delta_ratio_portfolio_history if config.ENABLE_SMA50_1Y_DELTA_RATIO else None),
            ...
            ("Static Price Curvature", static_price_curvature_portfolio_history if config.ENABLE_STATIC_PRICE_CURVATURE else None),
            ("Price Curvature",     price_curvature_portfolio_history     if config.ENABLE_PRICE_CURVATURE else None),
            ("Static Price Curvature 1Y Slope", static_price_curvature_1y_slope_portfolio_history if config.ENABLE_STATIC_PRICE_CURVATURE_1Y_SLOPE else None),
            ("Price Curvature 1Y Slope", price_curvature_1y_slope_portfolio_history if config.ENABLE_PRICE_CURVATURE_1Y_SLOPE else None),
            ("Confirmed Leaders",   confirmed_leaders_portfolio_history   if config.ENABLE_CONFIRMED_LEADERS else None),
            ("Foresight Mimic Accel", foresight_mimic_portfolio_history if config.ENABLE_FORESIGHT_MIMIC else None),
            ("BH 1Y / 1M Rank",     bh_1y_1m_rank_portfolio_history     if config.ENABLE_BH_1Y_1M_RANK else None),
            ("BH 1Y / 6M Rank",     bh_1y_6m_rank_portfolio_history     if config.ENABLE_BH_1Y_6M_RANK else None),
            ("BH 1Y / 6M Blend",    bh_1y_6m_blend_portfolio_history    if config.ENABLE_BH_1Y_6M_BLEND else None),
            ("BH 1Y SMA200",        bh_1y_sma200_portfolio_history      if config.ENABLE_BH_1Y_SMA200 else None),
            ("BH 1Y / FCF Rank",    bh_1y_fcf_rank_portfolio_history    if config.ENABLE_BH_1Y_FCF_RANK else None),
            ...
            ("BH 6M Perf Trig",     static_bh_6m_perf_portfolio_history if config.ENABLE_STATIC_BH_6M_PERFORMANCE else None),
            ("BH 9M Perf Trig",     static_bh_9m_perf_portfolio_history if config.ENABLE_STATIC_BH_9M_PERFORMANCE else None),
        ]
    ...
    results = {
        ...
        'strategies': {
            ...
            'sma50_1y_delta_ratio':     _strat(sma50_1y_delta_ratio_portfolio_value, sma50_1y_delta_ratio_portfolio_history, sma50_1y_delta_ratio_transaction_costs, sma50_1y_delta_ratio_cash),
            ...
            'static_price_curvature':  _strat(static_price_curvature_portfolio_value, static_price_curvature_portfolio_history, static_price_curvature_transaction_costs, static_price_curvature_cash),
            ...
            'static_price_curvature_1y_slope': _strat(static_price_curvature_1y_slope_portfolio_value, static_price_curvature_1y_slope_portfolio_history, static_price_curvature_1y_slope_transaction_costs, static_price_curvature_1y_slope_cash),
            'price_curvature_1y_slope': _strat(price_curvature_1y_slope_portfolio_value, price_curvature_1y_slope_portfolio_history, price_curvature_1y_slope_transaction_costs, price_curvature_1y_slope_cash),
            ...
            'foresight_mimic':         _strat(foresight_mimic_portfolio_value, foresight_mimic_portfolio_history, foresight_mimic_transaction_costs, foresight_mimic_cash),
            ...
            'deep_recovery':            _strat(deep_recovery_portfolio_value, deep_recovery_portfolio_history, deep_recovery_transaction_costs, deep_recovery_cash),
            ...
            'voting_ensemble_ai_rebalance': _strat(voting_ensemble_ai_rebalance_portfolio_value, voting_ensemble_ai_rebalance_portfolio_history, voting_ensemble_ai_rebalance_transaction_costs, voting_ensemble_ai_rebalance_cash) if config.ENABLE_VOTING_ENSEMBLE_AI_REBALANCE else _strat(0, [], 0, 0),
            ...
            'static_bh_6m_perf':        _strat(static_bh_6m_perf_portfolio_value, static_bh_6m_perf_portfolio_history, static_bh_6m_perf_transaction_costs, static_bh_6m_perf_cash) if config.ENABLE_STATIC_BH_6M_PERFORMANCE else _strat(0, [], 0, 0),
            'static_bh_9m_perf':        _strat(static_bh_9m_perf_portfolio_value, static_bh_9m_perf_portfolio_history, static_bh_9m_perf_transaction_costs, static_bh_9m_perf_cash) if config.ENABLE_STATIC_BH_9M_PERFORMANCE else _strat(0, [], 0, 0),
    ...
    if config.ENABLE_JSON_OUTPUT:
        ...
        strategy_selections = {
            ...
            'deep_recovery': {'tickers': list(current_deep_recovery_stocks), 'positions': {t: {'shares': p['shares'], 'avg_price': p.get('entry_price', p.get('avg_price', 0))} for t, p in deep_recovery_positions.items()}},
            'static_price_curvature': {'tickers': list(current_static_price_curvature_stocks), 'positions': {t: {'shares': p['shares'], 'avg_price': p.get('entry_price', p.get('avg_price', 0))} for t, p in static_price_curvature_positions.items()}},
            ...
            'static_price_curvature_1y_slope': {'tickers': list(current_static_price_curvature_1y_slope_stocks), 'positions': {t: {'shares': p['shares'], 'avg_price': p.get('entry_price', p.get('avg_price', 0))} for t, p in static_price_curvature_1y_slope_positions.items()}},
            'price_curvature_1y_slope': {'tickers': list(current_price_curvature_1y_slope_stocks), 'positions': {t: {'shares': p['shares'], 'avg_price': p.get('entry_price', p.get('avg_price', 0))} for t, p in price_curvature_1y_slope_positions.items()}},
            ...
            'foresight_mimic': {'tickers': list(current_foresight_mimic_stocks), 'positions': {t: {'shares': p['shares'], 'avg_price': p.get('entry_price', p.get('avg_price', 0))} for t, p in foresight_mimic_positions.items()}},
            ...
            'ratio_1m_3m': {'tickers': list(current_ratio_1m_3m_stocks), 'positions': {t: {'shares': p['shares'], 'avg_price': p.get('entry_price', p.get('avg_price', 0))} for t, p in ratio_1m_3m_positions.items()}},
            'sma50_1y_delta_ratio': {'tickers': list(current_sma50_1y_delta_ratio_stocks), 'positions': {t: {'shares': p['shares'], 'avg_price': p.get('entry_price', p.get('avg_price', 0))} for t, p in sma50_1y_delta_ratio_positions.items()}},
            ...
            'static_bh_6m_perf': {'tickers': list(current_static_bh_6m_perf_stocks), 'positions': {t: {'shares': p['shares'], 'avg_price': p.get('entry_price', p.get('avg_price', 0))} for t, p in static_bh_6m_perf_positions.items()}},
            'static_bh_9m_perf': {'tickers': list(current_static_bh_9m_perf_stocks), 'positions': {t: {'shares': p['shares'], 'avg_price': p.get('entry_price', p.get('avg_price', 0))} for t, p in static_bh_9m_perf_positions.items()}},
        ...
        try:
            ...
            if (config.ENABLE_VOTING_ENSEMBLE or config.ENABLE_VOTING_ENSEMBLE_AI_REBALANCE) and 'voting_ensemble' in results['strategies']:
                try:
                    live_voting_source_strategy_data = {}
                    for strategy_key, strategy_result in results['strategies'].items():
                        if strategy_key in {'voting_ensemble', 'voting_ensemble_ai_rebalance'}:
                            continue
                    ...
                    if config.ENABLE_VOTING_ENSEMBLE_AI_REBALANCE:
                        live_trading_selections['strategies']['voting_ensemble_ai_rebalance'] = list(voting_live_stocks)
    ...
    if voting_ensemble_ai_rebalance_cache_context:
        try:
            from ai_rebalance_strategy import cleanup_rebalance_training_context as _cleanup_voting_ai_rebalance_training_context
